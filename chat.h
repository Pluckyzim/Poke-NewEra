...
enum ChannelFlags_t
{
	...
	CHANNELFLAG_LOCKED = 1 << 3,
	CHANNELFLAG_PRIVATE = 1 << 4, // Only by system invite
	CHANNELFLAG_TALKEVENT = 1 << 5, // Raise onTalkChannel
};

typedef std::map<uint32_t, Player*> UsersMap;
typedef std::list<uint32_t> InviteList;
typedef std::list<Player*> ViewersList;
typedef std::list<uint32_t> BanList;
...
class ChatChannel
{
	public:
		...
		void internalTalk(const std::string& name, const std::string& text, SpeakClasses type);
		
		virtual TVChannel* getTVChannel() {return NULL;}
...
class TVChannel : public ChatChannel
{
    public:
        TVChannel(uint16_t id, std::string name, uint16_t flags);
        virtual ~TVChannel() {}
        
        virtual const uint32_t getOwner() {return m_owner;}
        void setOwner(uint32_t id) {m_owner = id;}
        
        bool addViewer(Player* player);
        bool removeViewer(Player* player);
        
        bool banViewer(Player* player);
        bool unbanViewer(Player* player);
        
        bool isBanned(Player* player);
        
        void closeChannel();
        
        virtual TVChannel* getTVChannel() {return this;}
        const ViewersList& getViewers() {return m_viewers;}
        ...
        void setTournamentChannel(bool value);
        bool getTournamentChannel();
        
        void replayDistanceShoot(const Position& from, const Position& to, uint8_t type);
        void replayMagicEffect(const Position& pos, uint16_t type);
		void replayAnimatedText(const Position& pos, uint8_t color, std::string text);
		void replayCreatureHealth(const Creature* creature);		     
        void replayCreatureTurn(const Creature* creature, int16_t stackpos);             
		void replayCreatureSay(const Creature* creature, SpeakClasses type, const std::string& text, Position* pos = NULL);
        void replayChangeSpeed(const Creature* creature, uint32_t speed);
        void replayCreatureOutfit(const Creature* creature, const Outfit_t& outfit);
        void replayCreatureSkull(const Creature* creature);
		void replayCreatureShield(const Creature* creature);
		void replayCreatureLight(const Creature* creature);		
		void replayWorldLight(const LightInfo& lightInfo);
		void replayAddTileItem(const Tile* tile, const Position& pos, uint32_t stackpos, const Item* item);
		void replayUpdateTileItem(const Tile* tile, const Position& pos, uint32_t stackpos, const Item* item);
		void replayRemoveTileItem(const Tile* tile, const Position& pos, uint32_t stackpos);
		void replayUpdateTile(const Tile* tile, const Position& pos);
		void replayAddCreature(const Creature* creature, const Position& pos, uint32_t stackpos);
		void replayRemoveCreature(const Creature* creature, const Position& pos, uint32_t stackpos);
		void replayUpdateCreature(const Creature* creature, const Position& pos, uint32_t stackpos);
		void replayMoveCreature(const Creature* creature, const Tile* newTile, const Position& newPos, uint32_t newStackpos, const Tile* oldTile, const Position& oldPos, uint32_t oldStackpos, bool teleport);
        
    protected:
        uint32_t m_owner;
        ViewersList m_viewers;
        BanList m_bans;
        std::string m_password;
        
    private:
        bool tournamentChannel;
};
...
{
	public:
		Chat(): statement(0), dummyPrivate(NULL), partyName("Party"), dummyTV(NULL) {}
		virtual ~Chat();
		...
		ChatChannel* addUserToChannel(Player* player, uint16_t channelId, bool forced = false);
		...
		TVChannel* getTVChannel(Player* player);
		TVChannel* getTVChannelById(uint16_t channelId);
		bool isTVChannel(uint16_t channelId) const {return m_TVChannels.find(channelId) != m_TVChannels.end();}
	...
	private:
		...
		typedef std::map<uint16_t, TVChannel*> TVChannelMap;
		TVChannelMap m_TVChannels;
