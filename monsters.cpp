...
{
	canPushItems = canPushCreatures = isSummonable = isIllusionable = isConvinceable = isLureable = isWalkable = hideName = hideHealth = isPacific = canWalkEverywhere = isShiny = isBoss = false;
	pushable = isAttackable = isHostile = isCatchable = true;

	outfit.lookHead = outfit.lookBody = outfit.lookLegs = outfit.lookFeet = outfit.lookType = outfit.lookTypeEx = outfit.lookAddons = 0;
	runAwayHealth = manaCost = sex = lightLevel = lightColor = yellSpeedTicks = yellChance = changeTargetSpeed = changeTargetChance = minLevel = maxLevel = 0;
...
void MonsterType::dropLoot(Container* corpse)
{
	...
	std::stringstream ss;
	ss << Localization::t(owner->getLanguage(), "Loot of ") << Localization::t(owner->getLanguage(), nameDescription) << ": " << corpse->getContentDescription() << ".";
...
bool Monsters::deserializeSpell(xmlNodePtr node, spellBlock_t& sb, const std::string& description)
{
	...
	else
	{
		...
		if(tmpName == "melee")
		{
			...
			{
				attack += 15; // PS - If the attack is less then 15, the damage can heal the target when on deffensive mode
                sb.minCombatValue = 0;
...
bool Monsters::loadMonster(const std::string& file, const std::string& monsterName, bool reloading/* = false*/)
{
	...
	if(readXMLString(root, "nickname", strValue))
	    mType->nickname = strValue;
    else
        mType->nickname = mType->name;
        
    if(readXMLString(root, "shiny", strValue))
        mType->shiny = strValue;
    else
        mType->shiny = "";
	...
	while(p && monsterLoad)
	{
		...
		if(!xmlStrcmp(p->name, (const xmlChar*)"health"))
		{
			...
			if(readXMLFloat(p, "rate", floatValue)) {
			    mType->healthRate = floatValue;
            }
		}
		else if(!xmlStrcmp(p->name, (const xmlChar*)"flags"))
		{
			...
			while(tmpNode)
			{
				...
				{	
                    if(readXMLString(tmpNode, "boss", strValue)) // PS
					    mType->isBoss = booleanString(strValue);
                    
                    if(readXMLString(tmpNode, "shiny", strValue)) // PS
					    mType->isShiny = booleanString(strValue);

					if(readXMLString(tmpNode, "catchable", strValue)) // PS
					    mType->isCatchable = booleanString(strValue);
                    
                    if(readXMLString(tmpNode, "canwalkeverywhere", strValue)) // PS
						mType->canWalkEverywhere = booleanString(strValue);
                    
                    if(readXMLString(tmpNode, "pacific", strValue)) // PS
						mType->isPacific = booleanString(strValue);
