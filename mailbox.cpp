...
bool Mailbox::sendItem(Creature* actor, Item* item)
{
	uint32_t depotId = 0;
	std::string name;
	if(!getRecipient(item, name, depotId) || name.empty()/* PS || !depotId*/)
		return false;
...
bool Mailbox::getRecipient(Item* item, std::string& name, uint32_t& depotId)
{
	...
	while(getline(iss, tmp, '\n') && curLine < 2)
	{
		if(curLine == 0)
			name = tmp;
		/* PS else if(curLine == 1)
			townString = tmp;*/

		++curLine;
	}
	
	if(Towns::getInstance()->getTown(name)) // PS - Protect against player/town name mistake, dont send it to players with citie names
	    return false;

	trimString(name);
	return true;
	/* PS if(townString.empty())
		return false;*/

	/* PS trimString(townString);
