...
ItemType::ItemType()
{
	...
	forceTransferToDepot = rareEffect = blockItemDropOver = false;
	sellPrice = 0;
...
void Items::parseItemNode(xmlNodePtr itemNode, uint32_t id)
{
	...
	if (readXMLInteger(itemNode, "sellprice", intValue)) {
        it.sellPrice = intValue;
    }
	...
	while(itemAttributesNode)
	{
		if(readXMLString(itemAttributesNode, "key", strValue))
		{
			...
			else if(tmpStrValue == "forcetransfertodepot" || tmpStrValue == "forcetransfer")
			{
				if(readXMLInteger(itemAttributesNode, "value", intValue))
					it.forceTransferToDepot = (intValue != 0);
			}
			else if (tmpStrValue == "rareeffect") {
                if (readXMLInteger(itemAttributesNode, "value", intValue)) {
                    it.rareEffect = (intValue != 0);
                }
            }
            else if (tmpStrValue == "blockitemdropover") {
                if (readXMLInteger(itemAttributesNode, "value", intValue)) {
                    it.blockItemDropOver = (intValue != 0);
                }
            }
...
const ItemType& Items::getItemIdByClientId(int32_t spriteId) const
{
	...
	static ItemType dummyItemType; // use this for invalid ids
	return dummyItemType;*/
	/*IntegerMap::const_iterator it = reverseItemMap.find(spriteId);
