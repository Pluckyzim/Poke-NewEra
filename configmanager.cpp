...
bool ConfigManager::load()
{
	...
	m_confString[SERVER_NAME] = getGlobalString("serverName", "Gameworld");
	...
	m_confDouble[RATE_MONSTER_EXPERIENCE_MULTIPLIER] = getGlobalDouble("rateMonsterExperienceMultiplier", 10);
	m_confBool[ADDONS_PREMIUM] = getGlobalBool("addonsOnlyPremium", true);
	m_confNumber[SHINY_APPEAR_CHANCE] = getGlobalNumber("shinyAppearChance", 8192);
	m_confBool[DEATH_LOSS_EXPERIENCE] = getGlobalBool("deathLossExperience", true);
	m_confNumber[DEATH_PROTECTION] = getGlobalNumber("deathProtection", 0);
	m_confBool[UPDATE_HIGHSCORES] = getGlobalBool("updateHighscores", true);
	m_confDouble[RATE_CATCH] = getGlobalDouble("rateCatch", 1);
	m_confBool[ADVANCED_SAVE] = getGlobalBool("advancedSave", false);
	m_confBool[DISCONNECT_AT_EXIT] = getGlobalBool("disconnectAtExit", false);
	m_confNumber[DEFAULT_TOWN_ID] = getGlobalNumber("defaultTownId", 3);
	m_confNumber[STARTER_TOWN_ID] = getGlobalNumber("starterTownId", 34);
	m_confNumber[MARKET_OFFER_DURATION] = getGlobalNumber("marketOfferDuration",  30 * 24 * 60 * 60);
	m_confBool[MARKET_PREMIUM] = booleanString(getGlobalString("premiumToCreateMarketOffer", "yes"));
	...
	m_confNumber[MINIMUM_LEVEL_TO_POLL_VOTE] = getGlobalNumber("minimumLevelToPollVote", 25);
