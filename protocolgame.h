...
class ProtocolGame : public Protocol
{
	...
	private:
		...
		//
		bool canSeeIgnoringTV(uint16_t x, uint16_t y, uint16_t z) const;
		bool canSeeIgnoringTV(const Creature*) const;
		bool canSeeIgnoringTV(const Position& pos) const;
		...
		//
		void parseRequestPollWindow(NetworkMessage& msg);
		void parsePollVote(NetworkMessage& msg);
		...
		void sendTextWindow(uint32_t windowTextId, uint32_t itemId, const std::string& text, uint16_t maxLen, bool canWrite);
		void sendHouseWindow(uint32_t windowTextId, House* house, uint32_t listId, const std::string& text);

		void sendOutfitWindow();
		void sendCustomOutfitWindow(const Outfit_t& defaultOutfit, std::list<Outfit>& outfits);
		void sendQuests();
		...
		void sendUpdateCreature(const Creature* creature, const Position& pos, uint32_t stackpos);
		...
		//PS Start
		void sendPokemonSkills(uint16_t iconItemId, std::vector<uint16_t> skills);
		...
		void sendLootList(std::list<Item*>& lootList);
#ifdef __LOG_PACKETS__		
		void recordPacket(const std::string& packet);
#endif
