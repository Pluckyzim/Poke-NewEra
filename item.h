...
class Item : virtual public Thing, public ItemAttributes
{
	public:
		...
		static std::string getDescription(const ItemType& it, int32_t lookDistance, const Item* item = NULL, int32_t subType = -1, bool addArticle = true, Player* player = NULL);
		static std::string getNameDescription(const ItemType& it, const Item* item = NULL, int32_t subType = -1, bool addArticle = true, Player* player = NULL);
		static std::string getWeightDescription(double weight, bool stackable, uint32_t count = 1);

		virtual std::string getDescription(int32_t lookDistance, Player* player = NULL) const {return getDescription(items[id], lookDistance, this, -1, true, player);}
		...
		//void setUniqueOwner(std::string uniqueOwner) {setAttribute("uniqueowner", uniqueOwner);} // PS
		//std::string getUniqueOwner() const; // PS
		
		void setUniqueOwner(uint32_t uniqueOwnerGUID) {setAttribute("uowner", (int32_t)uniqueOwnerGUID);} // PS
		uint32_t getUniqueOwner(); // PS
		...
		void setExpirationTime(uint32_t value) {setAttribute("expirationtime", (int32_t)value);}
		...
		bool forceTransferToDepot() const {return items[id].forceTransferToDepot;}
		...
		void addCreatureReference(uint32_t creatureId); // A creature is referencing me, I need to warn him when I get deleted.
		void cleanCreatureReferences();
    ...
    private:
        std::list<uint32_t> m_creatureReference;
};
...
/*
inline std::string Item::getUniqueOwner() const
{
	const std::string* v = getStringAttribute("uniqueowner");
	if(v)
		return *v;
...
inline uint32_t Item::getUniqueOwner()
{
	const int32_t* v = getIntegerAttribute("uowner");
	if(v)
		return (uint32_t)*v;
...
inline uint32_t Item::getExpirationTime()
{
	const int32_t* v = getIntegerAttribute("expirationtime");
	if(v)
		return (uint32_t)*v;
