...
bool DBInsert::addRow(const std::string& row)
{
	...
	else if(size > 8192)
	{
		...
		m_buf = "(" + row + ")";
		m_rows = 1; // PS - Bugfix: With this line items/outfits/etc will stop to disappear - Explain: When call execute, this is setted to 0
	}
	else
		m_buf += ",(" + row + ")";
...
{
	if(!m_multiLine || m_buf.length() < 1 || !m_rows) // INSERTs were executed on-fly / no rows to execute
		return true;
