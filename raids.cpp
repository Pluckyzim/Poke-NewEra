...
void Raids::checkRaids()
{
	...
	if (getRunning()) {
        for(RaidList::iterator it = raidList.begin(); it != raidList.end(); ++it) {
		    if ((*it)->isEnabled() && (*it)->hasRef()) {
			    char buffer[150];
	            sprintf(buffer, "%s - Remaining %d objectives!", (*it)->getName().c_str(), (*it)->getRefCount());
                g_game.broadcastMessage(buffer, MSG_STATUS_WARNING);
            }
	    }
		return;
    }
	...
	for(MonsterSpawnList::const_iterator it = m_spawnList.begin(); it != m_spawnList.end(); it++)
	{
		...
		for(uint32_t i = 0; i < amount; ++i)
		{
			...
			for(int32_t t = 0; t < MAXIMUM_TRIES_PER_MONSTER; ++t)
			{
				...
				monster->setMasterPosition(monster->getPosition());
