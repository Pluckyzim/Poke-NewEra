...
enum CreatureEventType_t
{
	...
	CREATURE_EVENT_GAINEXPERIENCE,
	CREATURE_EVENT_ADDCONTAINERITEM,
	CREATURE_EVENT_REMOVECONTAINERITEM,
	MONSTER_EVENT_SPAWN,
	CREATURE_EVENT_TRADE_REQUEST_ACCEPT,
	CREATURE_EVENT_QUEST_INFO,
	CREATURE_EVENT_STATISTIC_CHANGE,
	CREATURE_EVENT_KICK_LOGIN,
	CREATURE_EVENT_CHANNEL_TALK,
	CREATURE_EVENT_TOURNAMENT_START,
	CREATURE_EVENT_TOURNAMENT_HISTORY,
	MONSTER_EVENT_SINGLE_SPAWN,
	CREATURE_EVENT_STAMINA_CHANGE,
	CREATURE_EVENT_EXTENDED_OPCODE, // otclient additional network opcodes
	CREATURE_EVENT_CUSTOM_OUTFIT
};
...
class CreatureEvents : public BaseEvents
{
	public:
		...
		void playerKickLogin(Player* player);
		void tournamentHistory(uint32_t tournamentHistoryId, Player* winner, Player* loser);
...
class CreatureEvent : public Event
{
	public:
		...
		uint32_t executeCustomOutfit(Creature* creature, const Outfit_t& outfit);
		...
		uint32_t executeGainExperience(Player* player, uint32_t gainExp, Item* referenceItem);
		uint32_t executeAddContainerItem(Player* player, Item* item);
		uint32_t executeRemoveContainerItem(Player* player, Item* item);
		uint32_t executeSpawn(Monster* monster);
		uint32_t executeTradeRequestAccept(Player* player, Player* target, Item*item, Item* targetItem);
		uint32_t executeQuestInfo(Player* player, uint16_t questId);
		uint32_t executeStatisticChange(Player* player, uint16_t statisticId, uint32_t newValue);
		uint32_t executeKickLogin(Player* player);
		uint32_t executeChannelTalk(Player* player, uint16_t channelId, const std::string& message);
		uint32_t executeTournamentStart(Player* player, uint16_t tournamentId);
		uint32_t executeTournamentHistory(uint32_t tournamentHistoryId, Player* winner, Player* loser);
		uint32_t executeSingleSpawn(Monster* monster);
		uint32_t executeStaminaChange(Player* player, uint64_t stamina, int64_t change);
		uint32_t executeExtendedOpcode(Player* player, uint8_t opcode, const std::string& buffer);
		//
