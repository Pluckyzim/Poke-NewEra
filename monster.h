...
class Monster : public Creature
{
	...
	public:
		...
		virtual std::string getDescription(int32_t lookDistance, Player* player = NULL) const; // PS

		virtual RaceType_t getRace() const {return mType->race;}
		virtual int32_t getArmor() const {return mType->armor;}
		virtual int32_t getDefense() const {return mType->defense + extraDef;}
		virtual MonsterType* getMonsterType() const {return mType;}
		virtual bool isPushable() const {return !getNpcMaster() && (mType->pushable && (baseSpeed > 0));}
		virtual bool isAttackable() const {return mType->isAttackable;}
		virtual bool isImmune(CombatType_t type) const;

		bool canPushItems() const {return mType->canPushItems && !isSummon();}
		bool canPushCreatures() const {return mType->canPushCreatures;}
		bool isHostile() const {return mType->isHostile;}
		bool isPacific() const {return mType->isPacific;}
		...
		uint16_t getSex() {return sex;}
		void setSex(uint16_t newSex);
		
		uint32_t getLevel() const {return level;}
		void setLevel(uint32_t newLevel);
		
		uint16_t getExtraPoints() const {return extraPoints;}
		void setExtraPoints(uint16_t newExtraPoints) {extraPoints = newExtraPoints;}
		
		int getExtraSpAtk() const {return extraSpAtk;}
		void addExtraSpAtk(int extraSpAtkAmount);
		
		int getExtraSpDef() const {return extraSpDef;}
		void addExtraSpDef(int extraSpDefAmount);
		
		float getVarPokeStat(pokeStats_t pokeStat) const {return varPokeStats[pokeStat];}
		void setVarPokeStat(pokeStats_t pokeStat, float modifier);
		
		bool getCanWalkEverywhere() const {return canWalkEverywhere;}
		...
		bool selectTargetWithoutCheck(Creature* creature);
		...
		bool isFleeing() const {return !isSummon() && getHealth() <= mType->runAwayHealth;}

		virtual BlockType_t blockHit(Creature* attacker, CombatType_t combatType, int32_t& damage,
			bool checkDefense = false, bool checkArmor = false);
			
		void setUniqueTarget(Creature* _uniqueTarget);
		bool isUniqueTarget(Creature* creature, bool ignoreSummon = false);
		
		void setTargetDistance(int32_t _targetDistance) {targetDistance = _targetDistance;}
		void setFightMode(fightMode_t mode);
		
		uint8_t getSpecialAbility() const {return specialAbility;}
		void setSpecialAbility(uint8_t _specialAbility) {specialAbility = _specialAbility;}
		
		void setExperienceRate(float v) {experienceRate = v;}
	...
	private:
		...
		float varPokeStats[POKESTAT_LAST + 1];
		...
		virtual uint64_t getLostExperience() const {
                double bonusExp = (double)mType->experience * level * experienceRate;
                return ((skillLoss ? mType->experience + (uint32_t)bonusExp : 0));}
