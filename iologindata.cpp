...
Account IOLoginData::loadAccount(uint32_t accountId, bool preLoad/* = false*/)
{
	...
	query << "SELECT `id`, `name`, `password`, `premdays`, `lastday`, `key`, `warnings`, `lang_id`, `client_id` FROM `accounts` WHERE `id` = " << accountId << " LIMIT 1";
	DBResult* result;
	...
	query << "SELECT `name` FROM `players` WHERE `account_id` = " << accountId << " AND `world_id` = " << g_config.getNumber(ConfigManager::WORLD_ID) << " AND `deleted` = 0 AND `hidden` = 0";
#else
	query << "SELECT `name`, `world_id` FROM `players` WHERE `account_id` = " << accountId << " AND `deleted` = 0 AND `hidden` = 0";
#endif
	if(!(result = db->storeQuery(query.str())))
		return account;
...
bool IOLoginData::setAccountClient(uint32_t accountId, uint16_t clientId)
{
	Database* db = Database::getInstance();
	DBQuery query;
	query << "UPDATE `accounts` SET `client_id` = " << clientId << " WHERE `id` = " << accountId << db->getUpdateLimiter();
	return db->executeQuery(query.str());
}
...
void IOLoginData::removePremium(Account account)
{
	...
	if(account.premiumDays > 0 && account.premiumDays < 65535 && !g_config.getBool(ConfigManager::FREE_PREMIUM)) // Only debit days if isnt free premium
	{
		...
		if(days > 0)
		{
			...
			if(account.premiumDays < 1)
	        {
                // PS - On premium end reset account characters position.
	            DBQuery query;
	            Database* db = Database::getInstance();
	            query << "UPDATE `players` SET `posx` = 0, `posy` = 0, `posz` = 0, `town_id` = " << g_config.getNumber(ConfigManager::DEFAULT_TOWN_ID) << " WHERE `account_id` = " << account.number << " AND `town_id` != " << g_config.getNumber(ConfigManager::STARTER_TOWN_ID);
...
bool IOLoginData::loadPlayer(Player* player, const std::string& name, bool preLoad /*= false*/)
{
	...
	player->pvpArenaFrags = result->getDataInt("pvparenafrags");
	player->pvpArenaDeaths = result->getDataInt("pvparenadeaths");
	player->tournamentScore = result->getDataInt("tournament_score");
	player->tournamentWeeklyScore = result->getDataInt("tournament_weekly_score");
	...
	if((result = db->storeQuery(query.str())))
	{
		...
		for(ItemMap::reverse_iterator rit = itemMap.rbegin(); rit != itemMap.rend(); ++rit)
		{
			...
			if (item->checkExpirationTime()) {
    			...
    			{
                    if(!lockerFound && item->getID() == ITEM_DEPOT) // PS
                        lockerFound = true;
		...
		if(!lockerFound && depot) // PS
		    depot->__internalAddThing(Item::CreateItem(ITEM_DEPOT));
...
{   
	...
	query << "`pvparenafrags` = " << player->getPvpArenaFrags() << ", ";
	query << "`pvparenadeaths` = " << player->getPvpArenaDeaths() << ", ";
	query << "`tournament_score` = " << player->getTournamentScore() << ", ";
	query << "`tournament_weekly_score` = " << player->getTournamentWeeklyScore() << ", ";
	...
	if(g_config.getBool(ConfigManager::INGAME_GUILD_MANAGEMENT))
	{
		...
		query << "`rank_id` = " << IOGuild::getInstance()->getRankIdByName(player->getGuildId(), player->getRankName()) << ", ";
	}
	...
	if(g_config.getBool(ConfigManager::INGAME_GUILD_MANAGEMENT))
	{
		...
		{
			if (IOGuild::getInstance()->guildExists(*it)) { // The guild may be disbanded, if we allow the iterate here, the save will fail and a dump bug will occurr
...
bool IOLoginData::createCharacter(uint32_t accountId, std::string characterName, int32_t vocationId, uint16_t sex)
{
	...
	uint16_t healthMax = 350, manaMax = 0, capMax = 6, lookType = 612;
	if(sex % 2)
		lookType = 611;
...
bool IOLoginData::isPlayerOnlineByAccount(uint32_t acc)
{
    Database* db = Database::getInstance();
    DBResult* result = NULL;
    DBQuery query;

    query << "SELECT COUNT(*) AS `co` FROM `players` WHERE `players`.`online` = 1 AND `players`.`account_id` = " << acc;
    
    if (!(result = db->storeQuery(query.str()))) {
        return false;
    }
    
    bool value = result->getDataInt("co") > 0;
    result->free();
    return value;
}
...
uint32_t IOLoginData::getAccountHighestLevel(uint32_t acc)
{
    Database* db = Database::getInstance();
    DBResult* result = NULL;
    DBQuery query;

    query << "SELECT MAX(`level`) as `maxlevel` FROM `players` WHERE `players`.`account_id` = " << acc;
    
    if (!(result = db->storeQuery(query.str()))) {
        return 0;
    }
    
    uint32_t value = result->getDataInt("maxlevel");
    result->free();
    return value;
}

PokemonInfoList IOLoginData::getPlayerPokemonTeam(const std::string& name)
{    
    ...
    uint32_t guid;
    getGuidByName(guid, const_cast<std::string&>(name), true);
    
    query.str("");
	query << "SELECT `slot`, `pokemon_number`, `description` FROM `player_pokemon` WHERE `player_id` = " << guid;
	if ((result = db->storeQuery(query.str()))) {
		do {
			pokemonInfoList.push_back(PokemonInfo(result->getDataInt("pokemon_number"), result->getDataString("description")));
        } while(result->next());
