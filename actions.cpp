...
{    
    /*
    if(item->getUniqueOwner() != "" && !player->isItemUniqueOwner(item->getUniqueOwner()))
	    return RET_YOUARENOTTHEOWNER;
    */
	    
    if(item->getUniqueOwner() > 0 && !player->isItemUniqueOwner(item->getUniqueOwner()))
	    return RET_YOUARENOTTHEOWNER;
	...
	{
		if(container->getCorpseOwner())
        {
            if(!player->canOpenCorpse(container->getCorpseOwner()) && g_config.getBool(ConfigManager::CHECK_CORPSE_OWNER))
			    return RET_YOUARENOTTHEOWNER;
            else if(player->getAutoLoot() && container->size() > 0)
            {
                bool lootCollected = true;
                Item* cItem = NULL;
                std::list<Item*> lootList;
                
                while ((cItem = container->getItem(0))) {
                    if (g_game.internalMoveItem(player, container, player, INDEX_WHEREEVER, cItem, cItem->getItemCount(), NULL, FLAG_NOLIMIT) != RET_NOERROR) {
                        lootCollected = false;
                        break;
                    } else {
                        lootList.push_back(cItem);
                    }
                }
                
                if (lootCollected) {
                    player->sendTextMessage(MSG_INFO_DESCR, "Loot collected.");
                    player->sendLootList(lootList);
                    return RET_NOERROR;
                }
                else {
...
bool Actions::useItem(Player* player, const Position& pos, uint8_t index, Item* item, bool force /* false */)
{
	if(!player->canDoAction() && !force)
		return false;
