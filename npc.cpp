	...
	Creature()
{
	...
	loaded = false; //this is called on reset, remove?
	reset();
...
void Npc::onPlayerLeave(Player* player, NpcState* state)
{
    ...
    if(it != shopPlayerList.end()) {
        player->closeShopWindow(); // Bugfix: I dont want to close player shop window while other NPC has opened it
    }
...
void Npc::doSay(const std::string& text, SpeakClasses type, Player* player)
{
	if(!player)
	{
		...
		g_game.internalCreatureSay(this, type, tmp, false, NULL, NULL, true);
	}
	else
	{
...
{    
	...
	if(walkTicks <= 0 || !isIdle || focusCreature || getTimeSinceLastMove() < walkTicks || hasCondition(CONDITION_SLEEP, 0))
        return false;
...
bool Npc::canWalkTo(const Position& fromPos, Direction dir)
{
	...
	if(!tile->getHouseTile() && getHouser()) // prevent leaving house if is a houser
	    return false;
