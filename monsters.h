...
class MonsterType
{
	public:
		...
		Item* createLoot(const LootBlock& lootBlock, Player* owner = NULL);
		bool createChildLoot(Container* parent, const LootBlock& lootBlock, Player* owner = NULL);

		bool isSummonable, isIllusionable, isConvinceable, isAttackable, isHostile, isPacific, isLureable,
			isWalkable, canPushItems, canPushCreatures, pushable, hideName, hideHealth, canWalkEverywhere, isCatchable,
            isShiny, isBoss;
		...
		int32_t defense, armor, health, healthMax, baseSpeed, lookCorpse, corpseUnique, corpseAction,
			...
			lightLevel, lightColor, changeTargetSpeed, changeTargetChance, minLevel, maxLevel;
		uint32_t yellChance, yellSpeedTicks, staticAttackChance, manaCost;
		uint64_t experience;
		
		float healthRate;

		std::string name, nameDescription, nickname, shiny;
