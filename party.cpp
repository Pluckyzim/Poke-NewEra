...
bool Party::canUseSharedExperience(const Player* player, uint32_t highestLevel/* = 0*/) const
{
	...
	if(!highestLevel)
	{
		highestLevel = getPlayerPokemonLevel(leader);
		for(PlayerVector::const_iterator it = memberList.begin(); it != memberList.end(); ++it)
		{
			if(getPlayerPokemonLevel(*it) > highestLevel)
				highestLevel = getPlayerPokemonLevel(*it);
		}
	}

	if(getPlayerPokemonLevel(player) < (uint32_t)std::ceil((double)highestLevel * g_config.getDouble(
...
bool Party::canEnableSharedExperience()
{
	...
	uint32_t highestLevel = getPlayerPokemonLevel(leader);
	for(PlayerVector::iterator it = memberList.begin(); it != memberList.end(); ++it)
	{
		if(getPlayerPokemonLevel(*it) > highestLevel)
			highestLevel = getPlayerPokemonLevel(*it);
...
uint32_t Party::getPlayerPokemonLevel(const Player* player) const
{
    ...
    Creature* tmpCreature = tmpPlayer->getFirstSummon();
    if (!tmpCreature) {
        return 0;
    }
    
    Monster* tmpMonster = tmpCreature->getMonster();
    if (!tmpMonster) {
        return 0;
    }
    
    return tmpMonster->getLevel();
}
