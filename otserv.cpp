...
bool argumentsHandler(StringVec args)
{
	...
	for(StringVec::iterator it = args.begin(); it != args.end(); ++it)
	{
		...
		if((*it) == "--version")
		{
			...
			"A server developed by PokeWish Team.\n";
			return false;
		}
...
void serverMain(void* param)
{
    ...
    s.sa_mask = sst;
    s.sa_flags = 0;
    sigaction(SIGILL, &s, NULL);   // illegal instruction
    sigaction(SIGSEGV, &s, NULL);  // segmentation fault
...
ServiceManager* services)
{
	...
	std::cout << "A server developed by PokeWish Team." << std::endl << std::endl;
...
*/
	...
	std::cout << ">> Loading polls" << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Loading polls");
	#endif
    Polls::getInstance()->load();
    
    std::cout << ">> Loading localization" << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Loading localization");
	#endif
	...
	std::cout << ">> Loading tournaments" << std::endl;
	#if defined(WINDOWS) && !defined(__CONSOLE__)
	SendMessage(GUI::getInstance()->m_statusBar, WM_SETTEXT, 0, (LPARAM)">> Loading tournaments");
	#endif
	if(!Tournaments::getInstance()->loadFromXML())
		startupErrorMessage("Unable to load tournaments!");
	else {
        Tournaments::getInstance()->checkWeeklyWinner();
    }
			...
			switch(LOWORD(wParam))
			{
				...
				case ID_MENU_MAIN_CLEARLOG:
				{
					if(g_game.getGameState() != GAME_STATE_STARTUP)
					{
						...
						std::cout << STATUS_SERVER_NAME << ", version " << STATUS_SERVER_VERSION << " (" << STATUS_SERVER_CODENAME << ")" << std::endl;
						std::cout << "Compiled with " << BOOST_COMPILER << " at " << __DATE__ << ", " << __TIME__ << "." << std::endl;
						std::cout << "A server developed by PokeWish Team." << std::endl;
						std::cout << std::endl;
					}
