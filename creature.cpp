...
{
    return creature->isWalkable() || creature->isGhost() ||
        (getPlayerMaster() && // lets separate to detect where is the valgrind error
        creature->getPlayer());
}
...
{
	if(hasCondition(CONDITION_SLEEP, 0) || !attackedCreature || attackedCreature->getHealth() < 1)
		return;
...
void Creature::onWalk(Direction& dir)
{
	...
	uint32_t r = random_range(0, 8); /* PS From 16 to 8 */
	if(r > 4)
		return;
...
{
    if(listWalkDir.empty() || hasCondition(CONDITION_SLEEP, 0))
		return false;
...
{
    if(hasCondition(CONDITION_SLEEP, 0))
        return false;
...
void Creature::internalCreatureDisappear(const Creature* creature, bool isLogout)
{
	...
	CountMap::iterator it = damageMap.find(creature->getID());
	if (it != damageMap.end() && it->second.referenceItem == NULL && creature->getPlayerMaster()) {
        Player* master = creature->getPlayerMaster();
        Item* newReferenceItem = master->getInventoryItem(SLOT_FEET); // PokeBall
        
        if (newReferenceItem) {
            // first check if there is a reference to this item, otherwise a multiplied exp bug will ocurr (calling many times the same exp gain)
            bool found = false;
            for (CountMap::iterator cit = damageMap.begin(); cit != damageMap.end(); ++cit) {
                if (cit->second.referenceItem == newReferenceItem) {
                    cit->second.total += it->second.total;
                    found = true;
                    break;
                }
            }                              
            ...
            if (!found) {                  
                newReferenceItem->addCreatureReference(getID());
                it->second.referenceItem = newReferenceItem;
                it->second.referenceItemOwner = master->getID();
            }
            else {
                damageMap.erase(it);
            }
	...
	const Tile* oldTile, const Position& oldPos, bool teleport)
{
	if(creature == this)
	{
		...
		if(!summons.empty())
		{
			...
			for(cit = summons.begin(); cit != summons.end(); ++cit)
			{
				...
				if(teleport || (std::abs(pos.z - newPos.z) > 2) || (std::max(std::abs((
					newPos.x) - pos.x), std::abs((newPos.y - 1) - pos.y)) > 8))
					despawnList.push_back((*cit));
			}

			for(cit = despawnList.begin(); cit != despawnList.end(); ++cit)
				g_game.internalTeleport((*cit), this->getPosition(), false);
		}
    ...
    if(creature == this->getMaster()){isFollowingPosition = false;}
...
void Creature::giveKillersExperience()
{
    Creature* tmp = NULL;
    for(CountMap::iterator it = damageMap.begin(); it != damageMap.end(); ++it)
	{
        ...
        else if (it->second.referenceItem && it->second.referenceItemOwner) {
            Player* master = g_game.getPlayerByID(it->second.referenceItemOwner);
            if (master) {
                master->onAttackedCreatureKilled(this, it->second.referenceItem);
            }
        }                  
	}
}
...
void Creature::dropCorpse(DeathList deathList)
{
	...
	Item* topItem = tile->getTopTopItem();
	if (topItem && topItem->isBlocking(NULL)) {
    	...
    	if (!ownerEntry.isNameKill()) {
    	    ...
    	    for (DeathList::iterator it = deathList.begin(); it != deathList.end(); ++it) {
    			if (owner) {
    	            if (getDamageRatio(owner) < getDamageRatio(it->getKillerCreature())) {
    ...
    */
    g_game.addMagicEffect(tile->getPosition(), 46); // PS
...
DeathList Creature::getKillers()
{
	...
	for(CountMap::const_iterator it = damageMap.begin(); it != damageMap.end(); ++it)
	{
		...
		if (!mdc && cb.referenceItemOwner > 0) { // If this creature died, lets fire the kill event to the owner (if exists)
            mdc = g_game.getCreatureByID(cb.referenceItemOwner);               
        }  
...
bool Creature::hasBeenAttacked(uint32_t attackerId, bool checkTicks) const
{
	CountMap::const_iterator it = damageMap.find(attackerId);
	if(it != damageMap.end())
		return !checkTicks || (OTSYS_TIME() - it->second.ticks) <= g_config.getNumber(ConfigManager::PZ_LOCKED);
...
void Creature::getPathToFollowPosition()
{    
    if(hasCondition(CONDITION_SLEEP, 0))
        return;

	if(g_game.getPathTo(this, followPosition, listWalkDir, -1))
	{
	    hasFollowPath = isFollowingPosition = true;
		startAutoWalk(listWalkDir);
	}
	else
		hasFollowPath = false;
...
bool Creature::setFollowPosition(Position position)
{
	if(followPosition == position)
		return true;

	if(position.z != getPosition().z || !canSee(position))
	{
		return false;
	}
...
double Creature::getDamageRatio(Creature* attacker, Item* referenceItem) const
{
	double totalDamage = 0, attackerDamage = 0;
	for(CountMap::const_iterator it = damageMap.begin(); it != damageMap.end(); ++it)
	{
		totalDamage += it->second.total;
		if(it->first == attacker->getID() || (it->second.referenceItem != NULL && it->second.referenceItem == referenceItem))
...
double Creature::getDamageRatioByMaster(Creature* master) const
{
	double totalDamage = 0, attackerDamage = 0;
	for (CountMap::const_iterator it = damageMap.begin(); it != damageMap.end(); ++it) {
		bool add = false;
		...
		Creature* tmp = g_game.getCreatureByID(it->first);
		if (tmp && tmp->getMaster() == master) {
            add = true;
        }
...
void Creature::addDamagePoints(Creature* attacker, int32_t damagePoints)
{
	...
	if (attacker && attacker->isSummon() && attacker->getMaster()->isSummon()) {
...
{
	/*if(getHealth() > 0) This gives free exp if player X hurt 99% and go out of this vision, player B come and kill 1% but receive 100% exp.
	{
		healMap.clear();
		damageMap.clear();
	}*/
...
void Creature::onAttackedCreatureKilled(Creature* target, Item* referenceItem)
{
	if(target == this)
		return;

	double gainExp = target->getGainedExperience(this, referenceItem);
	onGainExperience(gainExp, !target->getPlayer(), false, referenceItem);
}
...
void Creature::onGainExperience(double& gainExp, bool fromMonster, bool multiplied, Item* referenceItem)
{
	if(gainExp <= 0)
		return;

	if(master)
		master->onGainExperience(gainExp, fromMonster, multiplied, referenceItem);
...
{           
    for(ConditionList::iterator it = conditions.begin(); it != conditions.end();)
	{
        if((onlyPersistent && !(*it)->isPersistent()) ||
            (reason == CONDITIONEND_DEATH && (*it)->getType() == CONDITION_MUTED)) //PS never remove muted conditions, otherwise players can die and spam trade channel again
		{
            ++it;
			continue;
		}
...
std::string Creature::getDescription(int32_t lookDistance, Player* player/* = NULL*/) const
{
	return "a creature";
}
...
void Creature::updateName()
{
     g_game.addCreatureUpdateName(this);
}

bool Creature::isPlayerSummonTop() const
{
    if (master) {
        return master->isPlayerSummonTop();
    }
    return getPlayer();
}

Creature* Creature::getTopMaster()
{
    if (master) {
        return master->getTopMaster();
    }
    return this;
}

const Creature* Creature::getTopMaster() const
{
    if (master) {
        return master->getTopMaster();
    }
...
void Creature::onReferencedItemRemoved(Item* item)
{
    for (CountMap::iterator it = damageMap.begin(); it != damageMap.end(); ++it) {
		if (it->second.referenceItem and it->second.referenceItem == item) {
            it->second.referenceItem = NULL;
            it->second.referenceItemOwner = 0;
        }
	}
}
