...
bool IOGuild::isInvited(uint32_t guild, uint32_t guid)
{
	...
	DBQuery query;
	query << "SELECT 1 FROM `guild_invites` WHERE `player_id` = " << guid << " AND `guild_id`= " << guild << " LIMIT 1";
	if(!(result = db->storeQuery(query.str())))
		return false;
...
bool IOGuild::createGuildRank(uint32_t guild, const std::string& name)
{
    Database* db = Database::getInstance();
	DBQuery query;
	query << "INSERT INTO `guild_ranks` (`guild_id`, `name`, `level`) VALUES ('" << guild << "', '" << name << "', '" << GUILDLEVEL_MEMBER << "')";
	return db->executeQuery(query.str());
}
