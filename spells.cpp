...
ReturnValue Spell::CreateIllusion(Creature* creature, const Outfit_t outfit, int32_t time, uint32_t subId)
{
	ConditionOutfit* outfitCondition = new ConditionOutfit(CONDITIONID_COMBAT, CONDITION_OUTFIT, time, false, subId);
	if(!outfitCondition)
		return RET_NOTPOSSIBLE;
...
bool InstantSpell::castSpell(Creature* creature)
{
	...
	else if(casterTargetOrDirection)
	{
...
bool InstantSpell::SearchPlayer(const InstantSpell* spell, Creature* creature, const std::string& param)
{
	...
	std::stringstream ss;
	ss << targetPlayer->getName() << " " << g_game.getSearchString(player->getPosition(), targetPlayer->getPosition(), player, true, true) << ".";
