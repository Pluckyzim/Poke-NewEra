...
std::string Item::getDescription(const ItemType& it, int32_t lookDistance, const Item* item/* = NULL*/,
	int32_t subType/* = -1*/, bool addArticle/* = true*/, Player* player/* = null*/)
{
	std::stringstream s;
	s << getNameDescription(it, item, subType, addArticle, player);
	if(item)
	...
	else if(it.weaponType != WEAPON_NONE)
	{
		...
		if(it.abilities.speed)
		{
			...
			s << (player ? Localization::t(player->getLanguage(), "speed ") : "speed ") << std::showpos << (int32_t)(it.abilities.speed / 2) << std::noshowpos;
		}
	...
	else if(it.armor || (item && item->getArmor()) || it.showAttributes)
	{
		...
		s << (player ? Localization::t(player->getLanguage(), " (Key:") : " (Key:") << (item ? (int32_t)item->getActionId() : 0) << ")";
	else if(it.isFluidContainer())
	{
		if(subType > 0)
			s << (player ? Localization::t(player->getLanguage(), " of ") : " of ") << (items[subType].name.length() ? items[subType].name : "unknown");
		else
			s << (player ? Localization::t(player->getLanguage(), ". It is empty") : ". It is empty");
	}
	else if(it.isSplash())
	{
		s << (player ? Localization::t(player->getLanguage(), " of ") : " of ");
		if(subType > 0 && items[subType].name.length())
			s << items[subType].name;
		else
			s << (player ? Localization::t(player->getLanguage(), "unknown") : "unknown");
	}
	else if(it.allowDistRead)
	{
		...
		if(item && !item->getText().empty())
		{
			if(lookDistance <= 4)
			{
				...
				{
					s << item->getWriter() << (player ? Localization::t(player->getLanguage(), " wrote") : " wrote");
					time_t date = item->getDate();
					if(date > 0)
						s << (player ? Localization::t(player->getLanguage(), " on ") : " on ") << formatDate(date);

					s << ": ";
				}
				else
					s << (player ? Localization::t(player->getLanguage(), "You read: ") : "You read: ");

				std::string text = (player ? Localization::t(player->getLanguage(), item->getText()) : item->getText());
				s << text;
				if(!text.empty())
				{
				...
				s << (player ? Localization::t(player->getLanguage(), "You are too far away to read it") : "You are too far away to read it");
		}
		else
			s << (player ? Localization::t(player->getLanguage(), "Nothing is written on it") : "Nothing is written on it");
	}
		...
		s << (player ? Localization::t(player->getLanguage(), " for level ") : " for level ") << item->getActionId() - it.levelDoor;
	...
	if (tmpItem && tmpItem->getUniqueOwner() > 0) {
        s << std::endl << (player ? Localization::t(player->getLanguage(), "It's an unique item.") : "It's an unique item.");
    }
    
    if (it.sellPrice > 0) {
        if (tmpItem) {
            s << std::endl << (player ? Localization::t(player->getLanguage(), "Sell price: $") : "Sell price: $") << (it.sellPrice * tmpItem->getItemCount());
        
            if (tmpItem->getItemCount() > 1) {
                 s << " (" << it.sellPrice << (player ? Localization::t(player->getLanguage(), " ea)") : " ea)");
            }
        
            s << ".";
        }
        else {
            s << std::endl << (player ? Localization::t(player->getLanguage(), "Sell price: $") : "Sell price: $") << it.sellPrice << ".";;
        }
    }
	...
	if(item && !item->getSpecialDescription().empty())
		...
		str = (player ? Localization::t(player->getLanguage(), it.description) : it.description);
...
std::string Item::getNameDescription(const ItemType& it, const Item* item/* = NULL*/, int32_t subType/* = -1*/, bool addArticle/* = true*/, Player* player/* = NULL*/)
{
	if(item)
		subType = item->getSubType();
	...
	if(it.name.length() || (item && item->getName().length()))
	{
		...
		else
		{
			if(addArticle)
			{
				if(item && !item->getArticle().empty())
					s << (player ? Localization::t(player->getLanguage(), item->getArticle()) : item->getArticle()) << " ";
				else if(!it.article.empty())
					s << (player ? Localization::t(player->getLanguage(), it.article) : it.article) << " ";
			}
		...
		s << (player ? Localization::t(player->getLanguage(), "nothing important") : "nothing important");
...
void Item::addCreatureReference(uint32_t creatureId)
{
    if (std::find(m_creatureReference.begin(), m_creatureReference.end(), creatureId) == m_creatureReference.end()) {
		m_creatureReference.push_back(creatureId);
    }
}

void Item::cleanCreatureReferences()
{
    Creature* tmpCreature = NULL;
    for (std::list<uint32_t>::const_iterator it = m_creatureReference.begin(), end = m_creatureReference.end(); it != end; ++it) {
        tmpCreature = g_game.getCreatureByID(*it);
        if (tmpCreature) {
            tmpCreature->onReferencedItemRemoved(this);
        }
    }
    m_creatureReference.clear();
}

bool Item::checkExpirationTime()
{
    uint32_t expirationTime = getExpirationTime();
    return expirationTime == 0 || expirationTime > time(NULL);
}
