...
typedef std::map<uint8_t, PvpArena*> PvpArenaMap;
typedef std::vector<PartyDuel*> PartyDuelVector;
typedef std::map<Tile*, Container*> BrowseFieldMap;

#define EVENT_PARTYDUELINTERVAL 500 /* If change this, change PartyDuel::onThink value too  */
#define EVENT_LOGONLINEPLAYERSINTERVAL 900000
/* #define EVENT_LOGPINGINTERVAL 1200000 */
...
class Game
{
	public:
		...
		std::string getTradeErrorDescription(ReturnValue ret, Item* item, Player* player);
		...
		ReturnValue placeSummon(Creature* creature, const std::string& name, bool forced = false, Position* pos = NULL);
		...
		uint32_t getWorldLightState() {return lightState;}
		...
		Item* findItemOfTypeWithAttribute(Cylinder* cylinder, uint16_t itemId,
			bool depthSearch = true, int32_t subType = -1, std::string attribute_key = "", boost::any attribute_value = -1);
			
		Item* findItemOfAttribute(Cylinder* cylinder, bool depthSearch = true,
            std::string attribute_key = "", boost::any attribute_value = -1);
		...
		bool playerRequestPollWindow(uint32_t playerId);
		bool playerPollVote(uint32_t playerId, uint8_t pollVote);
		bool playerPollVoteText(uint32_t playerId, std::string text);
		...
		bool isSightDirectClear(const Position& fromPos, const Position& toPos, bool floorCheck);
		...
		bool getPathToEx(const Creature* creature, const Position& targetPos, std::list<Direction>& dirList,
			const FindPathParams& fpp, bool extendedNodes = false);
		...
		std::string getSearchString(const Position fromPos, const Position toPos, Player* player, bool fromIsCreature = false, bool toIsCreature = false);
		...
		void addCreatureUpdateName(const Creature* creature);
		...
		void addCreatureJump(const Creature* creature);
		void addCreatureEffect(const Creature* creature, uint8_t effectId, uint32_t var);
		...
		int32_t getRealLightHour();
		void startDecay(Item* item);
		
		bool publicPlayerTalkToChannel(Player* player, SpeakClasses type, const std::string& text, uint16_t channelId); // PS
		
		bool partyDuelJoin(uint32_t leaderId, uint32_t invitedId); // PS
		bool partyDuelInvite(uint32_t leaderId, uint32_t invitedId); // PS
		bool partyDuelLeave(uint32_t playerId); // PS
		bool partyDuelCreate(uint32_t leaderId, uint8_t maxMembers, uint8_t maxPokemon, bool cancelable = true, TournamentBattle* tournamentBattle = NULL, uint32_t bet = 0); // PS
		
		PvpArena* getPvpArenaById(uint8_t id); // PS
        bool pvpArenaJoin(uint32_t playerId, uint8_t arenaId, uint8_t teamId); // PS
		bool pvpArenaLeave(uint32_t playerId); // PS
		bool pvpArenaCreate(uint8_t id, const Position& blueSpawn, const Position& redSpawn, const Position& exit, uint32_t maxLevel = 0, uint16_t blueChannel = 0, uint16_t redChannel = 0); // PS

        void emergencySave();
        Creature* getCreatureByNickname(std::string s);
        ReturnValue swapItems(Item* itemA, Item* itemB);
        void logOnlinePlayers();
      /*  void logPing(); */
        void playerAddStatistic(Player* player, PlayerStatistic_t statistic, uint32_t value);
	...
	protected:
		...
		PvpArenaMap pvpArenaMap;
		PartyDuelVector partyDuels;
