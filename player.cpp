	...
	Creature(), transferContainer(ITEM_LOCKER), name(_name), nameDescription(_name), client(p)
{
	...
	pzLocked = isConnecting = addAttackSkillPoint = requestedOutfit = autoTrade = watchingTV = recordingTV = dueling = battleing = tournamentReady = leaving = surviveArena = isAfk = isDashWalking = requestedCustomOutfit = false;
	saving = disconnectAtExit = autoLoot = true;
	...
	conditionImmunities = conditionSuppressions = groupId = vocation_id = managerNumber2 = town = skullEnd = pvpArenaFrags = pvpArenaDeaths = tournamentScore = tournamentWeeklyScore = 0;
	lastLogin = lastLogout = lastIP = messageTicks = messageBuffer = nextAction = 0;
	editListId = maxWriteLen = windowTextId = rankId = 0;
	duelPokemonRemaing = pvpArenaTeam = 0;
	pvpArenaHotFrags = 0;
	extraExpRate = 0;
	lastTvChannelId = 0;
...
{
	return accountManager == MANAGER_NONE && !hasFlag(PlayerFlag_CannotBePushed) && Creature::isPushable() && !watchingTV;
}

std::string Player::getDescription(int32_t lookDistance, Player* player/* = NULL*/) const
{
	std::stringstream s;
	if(lookDistance == -1)
	{
		...
		if(hasFlag(PlayerFlag_ShowGroupNameInsteadOfVocation))
			s << Localization::t(language, " You are ") << group->getName();
		else if(vocation_id != 0)
			s << Localization::t(language, " You are ") << vocation->getDescription();
		else
	...
	else
	{
		...
		if(!hasCustomFlag(PlayerCustomFlag_HideLevel))
			s << (player ? Localization::t(player->getLanguage(), "(Level ") : "Level ") << level << ")";

		s << ". " << (sex % 2 ? (player ? Localization::t(player->getLanguage(), "He") : "He") : Localization::t(language, "She"));
		if(hasFlag(PlayerFlag_ShowGroupNameInsteadOfVocation))
			s << (player ? Localization::t(player->getLanguage(), " is ") : " is ") << group->getName();
		else if(vocation_id != 0)
			s << (player ? Localization::t(player->getLanguage(), " is ") : " is ") << vocation->getDescription();
		else
			s << (player ? Localization::t(player->getLanguage(), " has no vocation") : " has no vocation");

        Town* town = Towns::getInstance()->getTown(getTown());
        s << (player ? Localization::t(player->getLanguage(), " from ") : " from ") << town->getName();
	...
	if(marriage && IOLoginData::getInstance()->getNameByGuid(marriage, tmp))
	{
		...
		{
			if(lookDistance == -1)
				s << Localization::t(language, "and you are");
			else
				s << (player ? Localization::t(player->getLanguage(), "and is") : "and is");

			s << " ";
		}

		s << (sex % 2 ? (player ? Localization::t(player->getLanguage(), "husband") : "husband") : (player ? Localization::t(player->getLanguage(), "wife") : "wife")) << (player ? Localization::t(player->getLanguage(), " of ") : " of ") << tmp;
	}
	...
	{
		if(lookDistance == -1)
			s << Localization::t(language, " You are ");
		else
			s << " " << (sex % 2 ? (player ? Localization::t(player->getLanguage(), "He") : "He") : (player ? Localization::t(player->getLanguage(), "She") : "She")) << (player ? Localization::t(player->getLanguage(), " is ") : " is ");

		s << (rankName.empty() ? (player ? Localization::t(player->getLanguage(), "a member") : "a member") : rankName)<< (player ? Localization::t(player->getLanguage(), " of the ") : " of the ") << guildName;
		if(!guildNick.empty())
			s << " (" << guildNick << ")";
	...
	if(watchingTV)
	    s << " " << (sex % 2 ? (player ? Localization::t(player->getLanguage(), "He") : "He") : (player ? Localization::t(player->getLanguage(), "She") : "She")) << (player ? Localization::t(player->getLanguage(), " is watching TV.") : " is watching TV.");
	
	if(pvpArena)
        s << "\nPVP - Frags: " << pvpArenaFrags << "(" << pvpArenaHotFrags << "), " << (player ? Localization::t(player->getLanguage(), "Deaths: ") : "Deaths: ") << pvpArenaDeaths << ".";    
...
{
    int32_t armor = 5; // PS default player armor now its 5 * multiplier
...
void Player::addSkillAdvance(skills_t skill, uint32_t count, bool useMultiplier/* = true*/)
{
	...
	{
		if (gainCount >= 300) {
            std::cout << "[Warning] skill advanced breaked! Player: " << getName() << ", Skill: " <<
                skill << ", Count: " << count << ", UseMultiplier: " << useMultiplier;
            break;
        }
        ...
        s.str("");
		s << Localization::t(language, "Your ") << getSkillName(skill) << Localization::t(language, " has increased to ") << skills[skill][SKILL_LEVEL];
...
int32_t Player::getContainerID(const Container* container) const
{
	...
	{
		if(cl->second.container == container)
			return cl->first;
	}
...
void Player::addContainer(uint32_t cid, Container* container)
{
	...
	for(ContainerVector::iterator cl = containerVec.begin(); cl != containerVec.end(); ++cl)
	{
		if(cl->first == cid)
		{
            if (cl->second.container->getID() == ITEM_BROWSEFIELD) {
			    cl->second.container->unRef();
		    }
		    
			cl->second.container = container;
			return;
		}
	}
	...
	containerVec.push_back(std::make_pair(cid, openContainer));
}
...
void Player::closeContainer(uint32_t cid)
{
	for(ContainerVector::iterator cl = containerVec.begin(); cl != containerVec.end(); ++cl)
	{
		if(cl->first == cid)
		{
			if (cl->second.container && cl->second.container->getID() == ITEM_BROWSEFIELD) {
		        cl->second.container->unRef();
        	}
...
{
	return getGUID() == ownerId || (party && party->canOpenCorpse(ownerId)) || hasCustomFlag(PlayerCustomFlag_GamemasterPrivileges);
}

/*
bool Player::isItemUniqueOwner(std::string uniqueOwnerName)
{
	return getName() == uniqueOwnerName;
}
*/

bool Player::isItemUniqueOwner(uint32_t uniqueOwnerGUID)
{
     return getGUID() == uniqueOwnerGUID;
}

bool Player::isItemUniqueOwner(uint32_t uniqueOwnerGUID) const
{
     return getGUID() == uniqueOwnerGUID;
}
...
{
    if(tvOwner)
        return tvOwner->canSeeCreature(creature);
...
bool Player::canSeeCreatureIgnoringTV(const Creature* creature) const
{
    if(creature == this)
		return true;
...
bool Player::canWalkthrough(const Creature* creature) const
{
	...
	if(creature->getPlayerMaster()) // PS - anti-trap with player summon
        return true;
        
    if (creature->getNpc() && creature->getNpc()->isWalkable()) {
        return true;
    }
...
bool Player::addDepot(Depot* depot, uint32_t depotId)
{
	...
	depot->setMaxDepotLimit((group != NULL ? group->getDepotLimit(isPremium()) : 2000));
	return true;
}
...
void Player::sendCancelMessage(ReturnValue message) const
{
	switch(message)
	{
		...
		case RET_PUTTHISOBJECTINYOURHAND:
			sendCancel("Put this object in your backpack.");
			break;
		...
		case RET_NOTENOUGHCAPACITY:
			sendCancel("You cannot carry more.");//"This object is too heavy.");
			break;
		...
		case RET_PLAYERISPZLOCKED:
			sendCancel("You cannot enter a protection zone after attacking another creature.");
			break;
		...
		case RET_PLAYERISPZLOCKEDLEAVEPVPZONE:
			sendCancel("You cannot leave a pvp zone after attacking another creature.");
			break;

		case RET_PLAYERISPZLOCKEDENTERPVPZONE:
			sendCancel("You cannot enter a pvp zone after attacking another creature.");
			break;
...
{    
	...
	{
		if(cl->second.container == container) {
            uint16_t slot = cl->second.index;
    		if (container->getID() == ITEM_BROWSEFIELD) {
	    		uint16_t containerSize = container->size() - 1;
		    	uint16_t pageEnd = cl->second.index + container->capacity() - 1;
			...
			client->sendAddContainerItem(cl->first/*, slot*/, item);
        }
	}
}
...
void Player::onCreatureAppear(const Creature* creature)
{
	...
	if(!isGhost() || !hasCondition(CONDITION_GAMEMASTER))
	...
	IODatalog::getInstance()->logPlayerItems(guid, getItemsCount());
}
...
void Player::onCreatureDisappear(const Creature* creature, bool isLogout)
{
	...
	leaving = true;	
    lastLogout = time(NULL); // PS always update lastLogout, this solve the problem with players spamming on trade channel (spam, die, spam, ...)
    if(isLogout)
		loginPosition = getPosition();
		
	/* This isnt need at PokeWish ##
    Item* item = NULL;
    for (int32_t slot = SLOT_FIRST; slot < SLOT_LAST; ++slot) {
	    if (!(item = getInventoryItem((slots_t)slot))) {
		    continue;
    ...
    */ 
	...
	clearPartyDuelInvitations();
	if(partyDuel)
	    partyDuel->leave(this);
	    
    if(pvpArena)
        pvpArena->leave(this, false);
        
    if(tournament)
        tournament->onPlayerLeave(this);
    ...
    IODatalog::getInstance()->logPlayerItems(guid, getItemsCount(), false);
    IOLoginData::getInstance()->setAccountLanguage(getAccount(), getLanguage());
}
	...
	const Tile* oldTile, const Position& oldPos, bool teleport)
{
	...
	if((teleport || oldPos.z != newPos.z) && !hasCustomFlag(PlayerCustomFlag_CanStairhop))
	{
		...
		/* PS - Now used in Creature::onCreatureMove
		if(g_config.getBool(ConfigManager::TELEPORT_PLAYER_SUMMONS))
		{
		    const std::list<Creature*>& summons = this->getSummons();
    	    CreatureList::const_iterator it = summons.begin();

	        for(uint32_t i = 1; it != summons.end(); ++it, ++i)
	        {
                g_game.internalTeleport(*it, this->getPosition(), false);
	        }
        }*/
	}
}
...
void Player::onAddContainerItem(const Container* container, const Item* item)
{
    ...
    /*
    CreatureEventList addContainerItemEvents = getCreatureEvents(CREATURE_EVENT_ADDCONTAINERITEM);
	for(CreatureEventList::iterator it = addContainerItemEvents.begin(); it != addContainerItemEvents.end(); ++it)
	{
		(*it)->executeAddContainerItem(this, const_cast<Item*>(item));
	}
	*/
...
{
    /*
    CreatureEventList removeContainerItemEvents = getCreatureEvents(CREATURE_EVENT_REMOVECONTAINERITEM);
	for(CreatureEventList::iterator it = removeContainerItemEvents.begin(); it != removeContainerItemEvents.end(); ++it)
	{
		(*it)->executeRemoveContainerItem(this, const_cast<Item*>(item));
	}
	*/
...
void Player::onThink(uint32_t interval)
{
    ...
    if(timeNow - lastPing >= 5000)
	{
		...
		if(client)
            ...
            if (g_config.getBool(ConfigManager::STOP_ATTACK_AT_EXIT)) {
			    setAttackedCreature(NULL);
            }
            if (g_config.getBool(ConfigManager::DISCONNECT_AT_EXIT) &&
                    !isConnecting && !hasCondition(CONDITION_INVULNERABLE) && getDisconnectAtExit()) {
	...
	if(getDisconnectAtExit() && (timeNow - lastPong) >= 60000 && canLogout(true))
	{
		if(client)
...
void Player::removeMessageBuffer()
{
	...
	if(!hasFlag(PlayerFlag_CannotBeMuted) && maxBuffer != 0 && messageBuffer <= maxBuffer + 1)
	{
		if(++messageBuffer > maxBuffer)
		{
			...
			char buffer[50];
			sprintf(buffer, Localization::t(language, "You are muted for %d seconds.").c_str(), muteTime);
...
void Player::drainHealth(Creature* attacker, CombatType_t combatType, int32_t damage)
{
	...
	char buffer[150];
	if(attacker)
		sprintf(buffer, Localization::t(language, "You lose %d hitpoint%s due to an attack by %s.").c_str(), damage, (damage != 1 ? "s" : ""), attacker->getNameDescription().c_str());
	else
		sprintf(buffer, Localization::t(language, "You lose %d hitpoint%s.").c_str(), damage, (damage != 1 ? "s" : ""));
...
void Player::drainMana(Creature* attacker, CombatType_t combatType, int32_t damage)
{
	...
	char buffer[150];
	if(attacker)
		sprintf(buffer, Localization::t(language, "You lose %d mana blocking an attack by %s.").c_str(), damage, attacker->getNameDescription().c_str());
	else
		sprintf(buffer, Localization::t(language, "You lose %d mana.").c_str(), damage);
...
void Player::addManaSpent(uint64_t amount, bool useMultiplier/* = true*/)
{
	...
	while(manaSpent + amount >= nextReqMana)
	{
		...
		char advMsg[50];
		sprintf(advMsg, Localization::t(language, "You advanced to magic level %d.").c_str(), magLevel);
...
{
	char buffer[100];
	sprintf(buffer, Localization::t(language, "You received %d experience point(s).").c_str(), exp);
	sendTextMessage(MSG_EVENT_DEFAULT, buffer);
	...
	if(prevLevel != level)
	{
		...
		if (!isUsingOtclient()) { // The client will handle it by himself
            char advMsg[60];
		    sprintf(advMsg, Localization::t(language, "You advanced from Level %d to Level %d.").c_str(), prevLevel, level);
		    sendTextMessage(MSG_EVENT_ADVANCE, advMsg);
        }
	    ...
	    IODatalog::getInstance()->logPlayerLevelUp(getGUID(), prevLevel, level, time(NULL), getPosition());
	}
...
void Player::removeExperience(uint64_t exp, bool updateStats/* = true*/)
{
	...
	if(prevLevel != level)
	{
		...
		char advMsg[90];
		sprintf(advMsg, Localization::t(language, "You were downgraded from Level %d to Level %d.").c_str(), prevLevel, level);
		sendTextMessage(MSG_EVENT_ADVANCE, advMsg);
	}
...
bool Player::onDeath()
{
    ...
    if(deny || pvpArena)
	{
        ...
        if(pvpArena)
        {
            removeConditions(CONDITIONEND_ABORT);
            pvpArena->onDeath(this);

            DeathEntry ownerEntry = deathList[0];
            if(ownerEntry.isNameKill())
		        return false;
            
            Creature* killer = ownerEntry.getKillerCreature();
            for(DeathList::iterator it = deathList.begin(); it != deathList.end(); ++it)
	        {
                if(killer)
		        {
                    if(getDamageRatio(killer) < getDamageRatio(it->getKillerCreature()))
                        killer = it->getKillerCreature();
                }
            ...
            if(killer)
            {
                if(Player* playerKiller = killer->getPlayerMaster())
                    pvpArena->onKill(playerKiller, this);
            }
        }
        else
    ...
    {
        (*it)->executeDeath(this, createCorpse(getKillers()), getKillers());
    }      
    
    if(g_config.getBool(ConfigManager::DEATH_LOSS_EXPERIENCE) &&
        level > uint32_t(g_config.getNumber(ConfigManager::DEATH_PROTECTION)))
    {
    ...
    if (!WaitingList::getInstance()->canLogin(this)) { 
        removeConditions(CONDITIONEND_DEATH);
        g_game.removeCreature(this, false);
        kickPlayer(false, true);
    } else {
	...
	if(skillLoss)
	{
		...
		//PS sendReLoginWindow();
		g_game.removeCreature(this, false);
		this->kickPlayer(false, true);
	}
	else
	{
		setLossSkill(true);
...
Item* Player::createCorpse(DeathList deathList)
{
	...
	std::stringstream ss;
	ss << Localization::t(language, "You recognize ") << getNameDescription() << ". " << (sex % 2 ? Localization::t(language, "He") : Localization::t(language, "She")) << Localization::t(language, " was killed by ");
	if(deathList[0].isCreatureKill())
	{
		...
		if(deathList[0].getKillerCreature()->getMaster())
			ss << Localization::t(language, " summoned by ") << deathList[0].getKillerCreature()->getMaster()->getNameDescription();
	}
	else
	...
	if(deathList.size() > 1)
	{
		if(deathList[0].getKillerType() != deathList[1].getKillerType())
		{
			...
			{
				ss << Localization::t(language, " and by ") << deathList[1].getKillerCreature()->getNameDescription();
				if(deathList[1].getKillerCreature()->getMaster())
					ss << Localization::t(language, " summoned by ") << deathList[1].getKillerCreature()->getMaster()->getNameDescription();
			}
			else
				ss << Localization::t(language, " and by ") << deathList[1].getKillerName();
		}
...
void Player::addExhaust(uint32_t ticks, Exhaust_t type)
{
	if(this->hasCondition(CONDITION_EXHAUST))
        this->removeCondition(CONDITION_EXHAUST);
...
bool Player::addVIP(uint32_t _guid, std::string& name, bool isOnline, bool internal/* = false*/)
{
	...
	if(VIPList.size() > (group ? group->getMaxVips(isPremium()) : 200))
	{
		if(!internal)
...
void Player::autoCloseContainers(const Container* container)
{
	...
	for(ContainerVector::iterator it = containerVec.begin(); it != containerVec.end(); ++it)
	{
		Container* tmp = it->second.container;
		while(tmp != NULL)
		{
...
{    
    ...
    if (Item* tmpItem = const_cast<Item*>(item)) {
	    if (tmpItem->getUniqueOwner() > 0 && !isItemUniqueOwner(tmpItem->getUniqueOwner())) {
...
{    
    if(index == 0 /*drop to capacity window*/ || index == INDEX_WHEREEVER)
	{
		*destItem = NULL;
		...
		for(int32_t i = SLOT_FIRST; i < SLOT_LAST; ++i)
		{
			if(i != SLOT_NECKLACE && i != SLOT_BACKPACK && !inventory[i] && __queryAdd(i, item, item->getItemCount(), 0) == RET_NOERROR)
			{
				index = i;
				return this;
			}
		...
		for(int32_t i = SLOT_FIRST; i < SLOT_LAST; ++i)
		{
			...
			if(Container* container = dynamic_cast<Container*>(inventory[i]))
			{
				...
				{
					container->__queryDestination(index, item, destItem, flags);
		...
		for(std::list<std::pair<Container*, int32_t> >::iterator dit = deepList.begin(); dit != deepList.end(); ++dit)
		{
			...
			for(ItemList::const_iterator it = c->getItems(); it != c->getEnd(); ++it)
			{
				...
				if(Container* subContainer = dynamic_cast<Container*>(*it))
				{
					...
					{
						subContainer->__queryDestination(index, item, destItem, flags);
                        //index = INDEX_WHEREEVER;
						//*destItem = NULL;
						return subContainer;
					}
...
void Player::onAddItem(Item* item)
{
     CreatureEventList addContainerItemEvents = getCreatureEvents(CREATURE_EVENT_ADDCONTAINERITEM);
     for(CreatureEventList::iterator it = addContainerItemEvents.begin(); it != addContainerItemEvents.end(); ++it)
     {
         (*it)->executeAddContainerItem(this, item);
     }
}

void Player::onRemoveItem(Item* item)
{
     CreatureEventList removeContainerItemEvents = getCreatureEvents(CREATURE_EVENT_REMOVECONTAINERITEM);
     for(CreatureEventList::iterator it = removeContainerItemEvents.begin(); it != removeContainerItemEvents.end(); ++it)
     {
         (*it)->executeRemoveContainerItem(this, item);
     }
}
...
{    
	...
	else if(const Creature* creature = thing->getCreature())
	{
		...
		for(ContainerVector::iterator it = containerVec.begin(); it != containerVec.end(); ++it)
		{
			if(!Position::areInRange<1,1,0>(it->second.container->getPosition(), getPosition()))
				containers.push_back(it->second.container);
		}
...
void Player::setChaseMode(chaseMode_t mode)
{
	...
	updateSummonsTargetDistance();
}

void Player::setFightMode(fightMode_t mode)
{
    fightMode = mode;
    updateSummonsFightMode();
}
...
void Player::onAddCombatCondition(ConditionType_t type, bool hadCondition)
{
	...
	if(!tmp.empty())
		sendTextMessage(MSG_STATUS_DEFAULT, Localization::t(language, "You are ") + Localization::t(language, tmp) + ".");
}
...
{     
	...
	/*Player* targetPlayer = target->getPlayer(); // PS
	if(!targetPlayer)
		return;
...
bool Player::checkLoginDelay(uint32_t playerId) const
{
	return (!hasCustomFlag(PlayerCustomFlag_IgnoreLoginDelay) && OTSYS_TIME() <= (lastLoad + g_config.getNumber(
		ConfigManager::LOGIN_PROTECTION)) && !hasBeenAttacked(playerId, true));
}
...
void Player::onAttackedCreatureDrain(Creature* target, int32_t points)
{
	...
	char buffer[100];
	sprintf(buffer, Localization::t(language, "You deal %d damage to %s.").c_str(), points, target->getNameDescription().c_str());
...
void Player::onSummonAttackedCreatureDrain(Creature* summon, Creature* target, int32_t points)
{
	...
	if(party && target && (!target->getMaster() || !target->getMaster()->getPlayer()) // PS
		&& target->getMonster() && target->getMonster()->isHostile()) //we have fulfilled a requirement for shared experience
		getParty()->addPlayerDamageMonster(this, points);

	char buffer[100];
	sprintf(buffer, Localization::t(language, "Your %s deals %d damage to %s.").c_str(), summon->getName().c_str(), points, Localization::t(language, target->getNameDescription()).c_str());
	sendTextMessage(MSG_EVENT_DEFAULT, buffer);
}
...
void Player::onGainExperience(double& gainExp, bool fromMonster, bool multiplied, Item* referenceItem)
{    
	...
	{
		if(!(*it)->executeGainExperience(this, (uint32_t)gainExp, referenceItem))
		    return;
    }

	if(gainExperience(gainExp, fromMonster))
		Creature::onGainExperience(gainExp, fromMonster, true, referenceItem);
}
...
void Player::onGainSharedExperience(double& gainExp, bool fromMonster, bool multiplied)
{
	...
	for(CreatureEventList::iterator it = expEvents.begin(); it != expEvents.end(); ++it)
	{
		if(!(*it)->executeGainExperience(this, (uint32_t)gainExp, NULL))
		    return;
    }
...
bool Player::doChangeRandomOutfit()
{
    Outfit_t newOutfit = defaultOutfit;
    requestedOutfit = true;
    
    for (OutfitMap::const_iterator it = outfits.begin(); it != outfits.end(); ++it) {
        newOutfit.lookType =it->second.lookType;
        if (changeOutfit(newOutfit, true)) {
	...
	switch(accountManager)
	{
		case MANAGER_NAMELOCK:
		{
			if(!talkState[1])
			{
				...
				if(managerString.length() < 4)
					msg << Localization::t(language, "Your name you want is too short, please select a longer name.");
				else if(managerString.length() > 20)
					msg << Localization::t(language, "The name you want is too long, please select a shorter name.");
				else if(!isValidName(managerString))
					msg << Localization::t(language, "That name seems to contain invalid symbols, please choose another name.");
				else if(IOLoginData::getInstance()->playerExists(managerString, true))
					msg << Localization::t(language, "A player with that name already exists, please choose another name.");
				else
				{
					...
					if(tmp.substr(0, 4) != "god " && tmp.substr(0, 3) != "cm " && tmp.substr(0, 3) != "gm ")
					{
						talkState[1] = true;
						talkState[2] = true;
						msg << managerString << Localization::t(language, ", are you sure?");
					}
					else
						msg << Localization::t(language, "Your character is not a staff member, please tell me another name!");
				}
			}
			else if(checkText(text, "no") && talkState[2])
			{
				talkState[1] = talkState[2] = false;
				msg << Localization::t(language, "What else would you like to name your character?");
			}
			else if(checkText(text, "yes") && talkState[2])
			{
				if(!IOLoginData::getInstance()->playerExists(managerString, true))
				{
						...
						IOBan::getInstance()->removePlayerBanishment(tmp, PLAYERBAN_LOCK))
					{
						...
						talkState[1] = true;
						talkState[2] = false;
						msg << Localization::t(language, "Your character has been successfully renamed, you should now be able to login at it without any problems.");
					}
					else
					{
						talkState[1] = talkState[2] = false;
						msg << Localization::t(language, "Failed to change your name, please try again.");
					}
				}
				else
				{
				...
				msg << Localization::t(language, "Sorry, but I can't understand you, please try to repeat that!");
		...
		case MANAGER_ACCOUNT:
		{
			...
			if(checkText(text, "cancel") || (checkText(text, "account") && !talkState[1]))
			{
				...
				msg << Localization::t(language, "Do you want to change your 'password', request a 'recovery key', add a 'character', or 'delete' a character?");
			}
			else if(checkText(text, "delete") && talkState[1])
			{
				talkState[1] = false;
				talkState[2] = true;
				msg << Localization::t(language, "Which character would you like to delete?");
			}
			else if(talkState[2])
			{
				std::string tmp = text;
				...
				if(!isValidName(tmp, false))
					msg << Localization::t(language, "That name contains invalid characters, try to say your name again, you might have typed it wrong.");
				else
				{
					talkState[2] = false;
					talkState[3] = true;
					managerString = tmp;
					msg << Localization::t(language, "Do you really want to delete the character named ") << managerString << "?";
				}
			}
			else if(checkText(text, "yes") && talkState[3])
			{
				switch(IOLoginData::getInstance()->deleteCharacter(managerNumber, managerString))
				{
					case DELETE_INTERNAL:
						msg << Localization::t(language, "An error occured while deleting your character. Either the character does not belong to you or it doesn't exist.");
						break;

					case DELETE_SUCCESS:
						msg << Localization::t(language, "Your character has been deleted.");
						break;

					case DELETE_HOUSE:
						msg << Localization::t(language, "Your character owns a house. To make sure you really want to lose your house by deleting your character, you have to login and leave the house or pass it to someone else first.");
						break;

					case DELETE_LEADER:
						msg << Localization::t(language, "Your character is the leader of a guild. You need to disband or pass the leadership someone else to delete your character.");
						break;

					case DELETE_ONLINE:
						msg << Localization::t(language, "A character with that name is currently online, to delete a character it has to be offline.");
						break;
				}
			...
			else if(checkText(text, "no") && talkState[3])
			{
				talkState[1] = true;
				talkState[3] = false;
				msg << Localization::t(language, "Tell me what character you want to delete.");
			}
			else if(checkText(text, "password") && talkState[1])
			{
				talkState[1] = false;
				talkState[4] = true;
				msg << Localization::t(language, "Tell me your new password please.");
			}
			else if(talkState[4])
			{
				std::string tmp = text;
				trimString(tmp);
				if(tmp.length() < 6)
					msg << Localization::t(language, "That password is too short, at least 6 digits are required. Please select a longer password.");
				else if(!isValidPassword(tmp))
					msg << Localization::t(language, "Your password contains invalid characters... please tell me another one.");
				else
				{
					talkState[4] = false;
					talkState[5] = true;
					managerString = tmp;
					msg << Localization::t(language, "Should '") << managerString << Localization::t(language, "' be your new password?");
				}
			}
			else if(checkText(text, "yes") && talkState[5])
			{
				...
				msg << Localization::t(language, "Your password has been changed.");
			}
			else if(checkText(text, "no") && talkState[5])
			{
				...
				msg << Localization::t(language, "Then not.");
			}
			else if(checkText(text, "character") && talkState[1])
			{
				if(account.charList.size() <= 15)
				{
					talkState[1] = false;
					talkState[6] = true;
					msg << Localization::t(language, "What would you like as your character name?");
				}
				else
				{
					talkState[1] = true;
					...
					msg << Localization::t(language, "Your account reach the limit of 15 players, you can 'delete' a character if you want to create a new one.");
				}
			}
			else if(talkState[6])
			{
			...
			else if(checkText(text, "yes") && talkState[7])
			{
				talkState[7] = false;
				talkState[8] = true;
				msg << Localization::t(language, "Should your character be a 'male' or a 'female'.");
			}
			else if(talkState[8] && (checkText(text, "female") || checkText(text, "male")))
			{
				...
				{
					msg << Localization::t(language, "A female, are you sure?");
					managerSex = PLAYERSEX_FEMALE;
				}
				else
				{
					msg << Localization::t(language, "A male, are you sure?");
			...
			else if(checkText(text, "no") && talkState[9])
			{
				talkState[8] = true;
				talkState[9] = false;
				msg << Localization::t(language, "Tell me... would you like to be a 'male' or a 'female'?");
			}
			else if(checkText(text, "yes") && talkState[9])
			{
				if(g_config.getBool(ConfigManager::START_CHOOSEVOC))
				{
					...
					for(VocationsMap::iterator it = Vocations::getInstance()->getFirstVocation(); it != Vocations::getInstance()->getLastVocation(); ++it)
					{
						if(it->first == it->second->getFromVocation() && it->first != 0)
						{
							if(firstPart)
							{
								msg << Localization::t(language, "What do you want to be... ") << it->second->getDescription();
								firstPart = false;
							}
							else if(it->first - 1 != 0)
								msg << ", " << it->second->getDescription();
							else
								msg << Localization::t(language, " or ") << it->second->getDescription() << ".";
						}
					}
				}
				else if(!IOLoginData::getInstance()->playerExists(managerString, true))
				{
					...
					if(IOLoginData::getInstance()->createCharacter(managerNumber, managerString, managerNumber2, (uint16_t)managerSex))
						msg << Localization::t(language, "Your character has been created.");
					else
						msg << Localization::t(language, "Your character couldn't be created, please try again.");
				}
				else
				{
					talkState[6] = true;
			...
			else if(talkState[11])
			{
				for(VocationsMap::iterator it = Vocations::getInstance()->getFirstVocation(); it != Vocations::getInstance()->getLastVocation(); ++it)
				{
					...
					{
						msg << Localization::t(language, "So you would like to be ") << it->second->getDescription() << Localization::t(language, "... are you sure?");
				...
				if(msg.str().length() == 17)
					msg << Localization::t(language, "I don't understand what vocation you would like to be... could you please repeat it?");
			}
			...
			else if(checkText(text, "no") && talkState[12])
			{
				talkState[11] = true;
				talkState[12] = false;
				msg << Localization::t(language, "No? Then what would you like to be?");
			}
			else if(checkText(text, "recovery key") && talkState[1])
			{
				talkState[1] = false;
				talkState[10] = true;
				msg << Localization::t(language, "Would you like a recovery key?");
			}
			else if(checkText(text, "yes") && talkState[10])
			{
				if(account.recoveryKey != "0")
					msg << Localization::t(language, "Sorry, you already have a recovery key, for security reasons I may not give you a new one.");
				else
				{
					managerString = generateRecoveryKey(4, 4);
					IOLoginData::getInstance()->setRecoveryKey(managerNumber, managerString);
					msg << Localization::t(language, "Your recovery key is: ") << managerString << ".";
				}
			...
			else if(checkText(text, "no") && talkState[10])
			{
				...
				msg << Localization::t(language, "Please read the latest message that I have specified, I don't understand the current requested action.");
		...
		case MANAGER_NEW:
		{
			...
			{
				msg << Localization::t(language, "What would you like your password to be?");
				talkState[1] = true;
				talkState[2] = true;
			}
			else if(talkState[2])
			{
				...
				else
				{
					...
					talkState[2] = false;
					managerString = tmp;
					msg << managerString << Localization::t(language, " is it? 'yes' or 'no'?");
				}
			}
			else if(checkText(text, "yes") && talkState[3])
			{
				if(g_config.getBool(ConfigManager::GENERATE_ACCOUNT_NUMBER))
				{
					...
					if(id)
					{
						...
						msg << Localization::t(language, "Your account has been created, you may manage it now, but remember your account name: '")
							<< managerChar << Localization::t(language, "' and password: '") << managerString
							<< Localization::t(language, "'! If the account name is too hard to remember, please note it somewhere.");
					}
					else
						msg << Localization::t(language, "Your account could not be created, please try again.");
				...
				{
					msg << Localization::t(language, "What would you like your account name to be?");
					talkState[3] = false;
					talkState[4] = true;
				}
			}
			else if(checkText(text, "no") && talkState[3])
			{
				talkState[2] = true;
				talkState[3] = false;
				msg << Localization::t(language, "What would you like your password to be then?");
			}
			else if(talkState[4])
			{
				std::string tmp = text;
				trimString(tmp);
				if(tmp.length() < 3)
					msg << Localization::t(language, "That account name is too short, at least 3 digits are required. Please select a longer account name.");
				else if(tmp.length() > 25)
					msg << Localization::t(language, "That account name is too long, not more than 25 digits are required. Please select a shorter account name.");
				else if(!isValidAccountName(tmp))
					msg << Localization::t(language, "Your account name contains invalid characters, please choose another one.");
				else if(asLowerCaseString(tmp) == asLowerCaseString(managerString))
					msg << Localization::t(language, "Your account name cannot be same as password, please choose another one.");
				else
				{
					sprintf(managerChar, "%s", tmp.c_str());
					msg << managerChar << Localization::t(language, ", are you sure?");
					talkState[4] = false;
					talkState[5] = true;
				}
			}
			else if(checkText(text, "yes") && talkState[5])
			{
				...
				{
					msg << Localization::t(language, "An account with that name already exists, please try another account name.");
					talkState[4] = true;
					talkState[5] = false;
				}
			}
			else if(checkText(text, "no") && talkState[5])
			{
				talkState[5] = false;
				talkState[4] = true;
				msg << Localization::t(language, "What else would you like as your account name?");
			}
			else if(checkText(text, "recover") && !talkState[6])
			{
				talkState[6] = true;
				talkState[7] = true;
				msg << Localization::t(language, "What was your account name?");
			}
			else if(talkState[7])
			{
				managerString = text;
				if(IOLoginData::getInstance()->getAccountId(managerString, (uint32_t&)managerNumber))
				{
					talkState[7] = false;
					talkState[8] = true;
					msg << Localization::t(language, "What was your recovery key?");
				}
				else
				{
					msg << Localization::t(language, "Sorry, but account with such name doesn't exists.");
			...
			else if(talkState[8])
			{
				...
				if(IOLoginData::getInstance()->validRecoveryKey(managerNumber, managerString2) && managerString2 != "0")
				{
					...
					msg << Localization::t(language, "Correct! Your new password is: ") << managerChar << ".";
				}
				else
					msg << Localization::t(language, "Sorry, but this key doesn't match to account you gave me.");
...
void Player::setTVOwner(Player* newTVOwner)
{
    if(eventWalk)
		setFollowCreature(NULL);
...
void Player::setTVChannel(TVChannel* newTVChannel)
{     
     if(client)
         client->setTVChannel(newTVChannel);
         
     if(newTVChannel)
         setRecordingTV(true);
     else
         setRecordingTV(false);
}

void Player::setDuelPokemonRemaing(uint8_t _duelPokemonRemaing, bool reseting)
{
    duelPokemonRemaing = _duelPokemonRemaing;
    if(!reseting && duelPokemonRemaing == 0 && partyDuel)
        partyDuel->onPlayerLost(this);
}

bool Player::addPartyDuelInvitation(PartyDuel* partyDuel)
{
	if(!partyDuel)
		return false;

	PartyDuelList::iterator it = std::find(invitePartyDuelList.begin(), invitePartyDuelList.end(), partyDuel);
	if(it != invitePartyDuelList.end())
		return false;

	invitePartyDuelList.push_back(partyDuel);
	return true;
}

bool Player::removePartyDuelInvitation(PartyDuel* partyDuel)
{
	if(!partyDuel)
		return false;
	...
	if(it != invitePartyDuelList.end())
	{
		invitePartyDuelList.erase(it);
		return true;
	}
	return false;
}

void Player::clearPartyDuelInvitations()
{
	if(invitePartyDuelList.empty())
		return;

	PartyDuelList list;
	for(PartyDuelList::iterator it = invitePartyDuelList.begin(); it != invitePartyDuelList.end(); ++it)
		list.push_back(*it);

	invitePartyDuelList.clear();
	for(PartyDuelList::iterator it = list.begin(); it != list.end(); ++it)
		(*it)->removeInvite(this);
}

bool Player::isPartyDuelInviting(const Player* player) const
{
	if(!player || !getPartyDuel() || (getPartyDuel()->getLeaderA() != this && getPartyDuel()->getLeaderB() != this))
		return false;

	return getPartyDuel()->isPlayerInvited(player);
}

bool Player::isOpponentPartyDuel(const Player* player) const
{
	if(!player || !getPartyDuel() || !player->getPartyDuel())
		return false;
		
    return getPartyDuel()->isPlayerUser(player) && getPartyDuel()->getPlayerTeam(this) != getPartyDuel()->getPlayerTeam(player);
}

bool Player::canAcceptTradeItem(Item* tradeItem)
{    
    if(Container* itemContainer = tradeItem->getContainer())
    {
        if(itemContainer->isHoldingUniqueItem())
            return false;
    }
    
    else if(tradeItem->getUniqueOwner())
        return false;
    
    return true;
}

void Player::updateSummonsTargetDistance()
{
    if(getSummonCount())
	{
        int32_t targetDistance = (chaseMode ? 3 : 1);
        const std::list<Creature*>& summons = this->getSummons();
   	    CreatureList::const_iterator it = summons.begin();
        for(uint32_t i = 1; it != summons.end(); ++it, ++i)
        {
            if(Monster* tmpMonster = (*it)->getMonster())
                tmpMonster->setTargetDistance(targetDistance);
        }
    }                            
}

void Player::updateSummonsFightMode()
{
    if(getSummonCount())
	{
        const std::list<Creature*>& summons = this->getSummons();
        ...
        {
            if(Monster* tmpMonster = (*it)->getMonster())
                tmpMonster->setFightMode(fightMode);
        }
    }
}

void Player::addSummon(Creature* creature)
{
	Creature::addSummon(creature);
	updateSummonsTargetDistance();
	updateSummonsFightMode();
}

Creature* Player::getFirstSummon()
{
    if (getSummonCount()) {
        const std::list<Creature*>& summons = this->getSummons();
   	    CreatureList::const_iterator it = summons.begin();
...
uint32_t Player::getItemsCount() const
{
	Item* item = NULL;
	Container* container = NULL;
...
int64_t Player::getLastPing() const
{
    return lastPing;
}

int64_t Player::getLastPong() const
{
    return lastPong;
}

void Player::setSurviveArena(bool _surviveArena)
{
    surviveArena = _surviveArena;
}

bool Player::getSurviveArena() const
{
    return surviveArena;
}

void Player::setDisconnectAtExit(bool _disconnectAtExit)
{
    disconnectAtExit = _disconnectAtExit;
}

bool Player::getDisconnectAtExit() const
{
    return disconnectAtExit;
}
...
void Player::sendPollWindow() const
{
    if (!client) {
        return;
    }
    
    Poll* poll = Polls::getInstance()->getAvailablePollForAccount(getAccount());
    if (poll) {
        client->sendPollWindow(poll);
    }
}
...
void Player::useStamina(int64_t value) {
    stamina = std::min((int64_t)STAMINA_MAX, (int64_t)std::max((int64_t)0, ((int64_t)stamina + value)));
    CreatureEventList events = getCreatureEvents(CREATURE_EVENT_STAMINA_CHANGE);
		for(CreatureEventList::iterator it = events.begin(); it != events.end(); ++it)
			(*it)->executeStaminaChange(this, getStamina(), value);
}
