...
void ProtocolGame::sendContainer(uint32_t cid, const Container* container, bool hasParent)
{
	...
	if(msg)
	{
		...
		for (uint32_t i = 0; cit != container->getEnd() && i < 255; ++cit, ++i)
		{
			...
			{
				const int32_t* levelAttribute = item->getIntegerAttribute("10005");
				level = *levelAttribute;
			}
			std::string pokeName = "none";
			if (item->getStringAttribute("10002"))
			{
				const std::string* pokeNameAttribute = item->getStringAttribute("10002");
				pokeName = *pokeNameAttribute;
			}
			msg->AddU32(level);
			msg->AddString(pokeName);
		}
	}
}
...
void ProtocolGame::sendPokemonWindowAddPokemonIcon(uint16_t itemId, uint16_t fastcallNumber, uint8_t textColor, std::string text)
{
    if(OutputMessage_ptr output = OutputMessagePool::getInstance()->getOutputMessage(this, false)) //single message, with this doesn't need to parse in the client
    {
#ifdef __LOG_PACKETS__
...
void ProtocolGame::sendPokemonWindowUpdatePokemonIcon(uint16_t fastcallNumber, uint8_t textColor, std::string newText)
{
    if(OutputMessage_ptr output = OutputMessagePool::getInstance()->getOutputMessage(this, false)) //single, with this doesn't need to parse in the client
    {
#ifdef __LOG_PACKETS__
...
void ProtocolGame::sendQuests()
{
	...
	if(msg)
	{
		...
		msg->AddU16(Quests::getInstance()->getQuestCount());
...
void ProtocolGame::sendQuestInfo(Quest* quest)
{
	...
	if(msg)
	{
		...
		{
			if(!(*it)->isStarted(player) && it != quest->getFirstMission()) // The first one we always send
				continue;
			...
			if(player->isUsingOtclient()) {
                if ((*it)->getNpcName() != "") {
                    msg->AddByte(1);
                    msg->AddString((*it)->getNpcName());
                    msg->AddU16((*it)->getLevel());

                    msg->AddU16((*it)->getNpcOutfit().lookType);
                    msg->AddByte((*it)->getNpcOutfit().lookHead);
    			    msg->AddByte((*it)->getNpcOutfit().lookBody);
    			    msg->AddByte((*it)->getNpcOutfit().lookLegs);
    			    msg->AddByte((*it)->getNpcOutfit().lookFeet);
    			    msg->AddByte((*it)->getNpcOutfit().lookAddons);
			
                    msg->AddPosition((*it)->getNpcPosition());
                } else {
                    msg->AddByte(0);
                }
            }
...
void ProtocolGame::sendCustomQuestInfo(uint16_t questId, std::map<std::string, std::string> &missions)
{
	...
	if(msg)
	{
		...
		for(std::map<std::string, std::string>::const_iterator it = missions.begin(); it != missions.end(); ++it) {
			...
			if(player->isUsingOtclient()) {
                msg->AddByte(0); // Disable additional quest info
            }
		}
	}
}
