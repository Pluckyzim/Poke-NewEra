...
Container::Container(Tile* tile) : Item(ITEM_BROWSEFIELD)
{
	...
	if (itemVector) {
        for(ItemVector::const_iterator it = itemVector->begin(); it != itemVector->end(); ++it) {
			if ((*it)->getContainer()/* && (*it)->isMoveable())*/ && !(*it)->getUniqueId()) {
				itemlist.push_front((*it));
				(*it)->setParent(this);
			}
		}
	}
...
bool Container::isHoldingUniqueItem() const
{
    ...
    Thing* thing = NULL;
    Item* item = NULL;
    for(int32_t i = this->__getFirstIndex(); i < this->__getLastIndex();)
    {
        if((thing = this->__getThing(i)) && (item = thing->getItem()))
        {
...
{	
    if (this == getTopParent() && !getDepot()) {
        if (const Item* citem = thing->getItem()) {
            if (Item* item = const_cast<Item*>(citem)) {
                if (item->getUniqueOwner() > 0) {
	...
	if(index == INDEX_WHEREEVER && !((flags & FLAG_NOLIMIT) == FLAG_NOLIMIT) && full() && !canAutoGroup(thing->getItem(), index))
	...
	uint32_t& flags)
{
	...
	else
	{
		...
		// PS -- auto group items
        else if (const Item* moveItem = thing->getItem())
		{
            Item* tmpItem = canAutoGroup(moveItem, index);
            if (tmpItem) {
                *destItem = tmpItem;
            }
        }
...
Item* Container::canAutoGroup(const Item* moveItem, int32_t& index) const
{
    if (!moveItem->isStackable()) {
        return NULL;
    }
        
    if (Item* _moveItem = const_cast<Item*>(moveItem)) {
        Item* tmpItem = NULL;
        uint32_t n = 0;
        for (ItemList::const_iterator it = this->getItems(); it != this->getEnd(); ++it) {
            tmpItem = *it;
			if (tmpItem != moveItem && tmpItem->getID() == moveItem->getID() &&
                tmpItem->getUniqueOwner() == _moveItem->getUniqueOwner() && tmpItem->getItemCount() < 100) {
                index = n;
                return tmpItem;
            }
            ++n;
