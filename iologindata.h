...
typedef std::pair<uint16_t, std::string> PokemonInfo;
typedef std::list<PokemonInfo> PokemonInfoList;
...
class IOLoginData
{
	public:
		...
		uint32_t getAccountHighestLevel(uint32_t acc);
		...
		bool setAccountClient(uint32_t accountId, uint16_t clientId);
		
		PokemonInfoList getPlayerPokemonTeam(const std::string& name);
