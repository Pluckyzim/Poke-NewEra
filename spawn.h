...
class Spawn
{
	public:
		...
		bool addMonster(const std::string& _name, const Position& _pos, Direction _dir, uint32_t _interval, uint32_t level);
	...
	private:
		...
		void checkSpawn();
		bool spawnMonster(uint32_t spawnId, MonsterType* mType, const Position& pos, Direction dir, uint32_t level, bool startup = false);
