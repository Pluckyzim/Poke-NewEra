...
void Game::start(ServiceManager* servicer)
{
	...
	Scheduler::getInstance().addEvent(createSchedulerTask(EVENT_PARTYDUELINTERVAL,
        boost::bind(&Game::checkPartyDuels, this)));
    Scheduler::getInstance().addEvent(createSchedulerTask(EVENT_LOGONLINEPLAYERSINTERVAL,
        boost::bind(&Game::logOnlinePlayers, this)));
/*    Scheduler::getInstance().addEvent(createSchedulerTask(EVENT_LOGPINGINTERVAL,
        boost::bind(&Game::logPing, this))); */
	...
	uint32_t spriteId/* = 0*/, stackposType_t type/* = STACKPOS_NORMAL*/)
{
	...
	else if(pos.y & 0x40)
	{
		...
		if (parentContainer->getID() == ITEM_BROWSEFIELD) {
			...
			if (tile && tile->hasProperty(SUPPORTHANGABLE)) {
				if (tile->hasProperty(ISVERTICAL)) {
					if (player->getPosition().x + 1 == tile->getPosition().x) {
						return NULL;
					}
...
ReturnValue Game::placeSummon(Creature* creature, const std::string& name, bool forced, Position* pos)
{
    ...
    if (pos) {
        ...
        tmpPos = creature->getPosition();
    }

	// Place the monster
	creature->addSummon(monster);
	if(placeCreature(monster, tmpPos, true, forced)) // PS Added forced place with default value to false
		return RET_NOERROR;
	...
	uint16_t spriteId, int16_t fromStackpos, const Position& toPos, uint8_t count)
{
	...
	if(!player || player->isRemoved() || player->hasCondition(CONDITION_SLEEP, 0))
		return false;
...
if (creature->getMonster())
{
	...
	{
		internalCreatureTurn(creature, getDirectionTo(toCylinder->getTile()->getPosition(),
            subCylinder->getTile()->getPosition(), false)); // Wrong facing direction at stairs use http://otland.net/project.php?issueid=2301
...
{          
    /*if (fromCylinder) {
	    ...
	    if (fromTile) {
            BrowseFieldMap::iterator it = browseFields.find(fromTile);
	...
	if(toItem && toItem->getID() == item->getID() && item->isStackable() &&
       toItem->getUniqueOwner() != item->getUniqueOwner())
        return RET_NOTPOSSIBLE;
	...
	{        
		...
		if(m - n > 0)
		{
			...
			moveItem->setUniqueOwner(item->getUniqueOwner());
        }
		else
			moveItem = NULL;
...
Item* Game::findItemOfTypeWithAttribute(Cylinder* cylinder, uint16_t itemId,
	bool depthSearch /*= true*/, int32_t subType /*= -1*/, std::string attribute_key, boost::any attribute_value)
{
	if(!cylinder)
		return NULL;
	...
	Thing* thing = NULL;
	Item* item = NULL;
	bool isItemWithAttribute = false;
	for(int32_t i = cylinder->__getFirstIndex(); i < cylinder->__getLastIndex();)
	{
		if((thing = cylinder->__getThing(i)) && (item = thing->getItem()))
		{
            boost::any value = item->getAttribute(attribute_key);
	        if(value.empty())
                 isItemWithAttribute = false;
                 
            else if(value.type() == typeid(std::string) && attribute_value.type() == typeid(std::string) &&
                    boost::any_cast<std::string>(value) == boost::any_cast<std::string>(attribute_value))
                     isItemWithAttribute = true;
		             
           	else if(value.type() == typeid(int32_t) && attribute_value.type() == typeid(int32_t) &&
                    boost::any_cast<int32_t>(value) == boost::any_cast<int32_t>(attribute_value))
		             isItemWithAttribute = true;

            else if(value.type() == typeid(float) && attribute_value.type() == typeid(float) &&
                    boost::any_cast<float>(value) == boost::any_cast<float>(attribute_value))
		             isItemWithAttribute = true;
		             
            else if(value.type() == typeid(bool) && attribute_value.type() == typeid(bool) &&
                    boost::any_cast<bool>(value) == boost::any_cast<bool>(attribute_value))
		             isItemWithAttribute = true;   
            else { isItemWithAttribute = false; }
		        
			if(item->getID() == itemId && (subType == -1 || subType == item->getSubType()) && isItemWithAttribute)
				return item;
			else
			{
				++i;
...
Item* Game::findItemOfAttribute(Cylinder* cylinder, bool depthSearch /*= true*/,
    std::string attribute_key, boost::any attribute_value)
{
	if(!cylinder)
		return NULL;
	...
	for(IntegerMap::reverse_iterator it = moneyMap.rbegin(); it != moneyMap.rend(); ++it)
	{
		...
		if(tmp != 0)
		{
			do
			{
				...
				remaindItem->setParent(VirtualCylinder::virtualCylinder);
				
                if (internalMoveItem(NULL, remaindItem->getParent(), cylinder, SLOT_AMMO, remaindItem,
                      remaindItem->getItemCount(), NULL, FLAG_NOLIMIT) != RET_NOERROR) {
...
bool Game::playerMove(uint32_t playerId, Direction dir)
{
	...
	if(player->getNoMove() || player->hasCondition(CONDITION_SLEEP, 0))
	{
		player->sendCancelWalk();
		return false;
	}
	...
	if (player->getIsDashWalking() || !player->isUsingOtclient()) {
...
bool Game::playerOpenChannel(uint32_t playerId, uint16_t channelId)
{
    ...
    if (TVChannel* tvChannel = channel->getTVChannel())
    {
        if (tvChannel->getPassword() != "" && !player->hasCustomFlag(PlayerCustomFlag_GamemasterPrivileges)) {
            player->setLastTvChannelId(tvChannel->getId());
            player->sendTextWindow(ITEM_LETTER, "", 24, true);
            player->sendTextMessage(MSG_STATUS_CONSOLE_BLUE, "This is a private channel. Please enter the password.");
            return false;
        }
    }
...
{
//    std::cout << "playerUseItemEx - playerId:" << playerId << " - fromPos:" << fromPos << " - fromStackpos:" << fromStackpos <<
//                 " - fromSpriteId:" << fromSpriteId << " - toPos:" << toPos << " - toStackpos: " << toStackpos <<
//,                 " - toSpriteId: " << toSpriteId << " - isHotkey: " << isHotkey << std::endl;
	Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved() || player->hasCondition(CONDITION_SLEEP, 0))
		return false;
...
bool Game::playerMoveUpContainer(uint32_t playerId, uint8_t cid)
{
	...
	if(!parentContainer) {
		...
		BrowseFieldMap::iterator it = browseFields.find(tile);
		if (it == browseFields.end()) {
			parentContainer = new Container(tile);
			parentContainer->addRef();
			browseFields[tile] = parentContainer;
			Scheduler::getInstance().addEvent(createSchedulerTask(30000, boost::bind(&Game::decreaseBrowseFieldRef, this, tile->getPosition())));
		} else {
...
bool Game::playerWriteItem(uint32_t playerId, uint32_t windowTextId, const std::string& text)
{
	...
	if (!writeItem && player->getLastTvChannelId() > 0) {
        if (TVChannel* tv = g_chat.getTVChannelById(player->getLastTvChannelId())) {
            if (tv->getPassword() == text) {
                tv->addUser(player);
                player->sendChannel(tv->getId(), tv->getName());
            }
            else {
                player->sendCancelMessage(RET_NOTPOSSIBLE);
            }
...
void Game::playerBrowseField(uint32_t playerId, const Position& pos)
{
	...
	if (!Position::areInRange<1, 1>(playerPos, pos)) {
		...
		if(getPathToEx(player, pos, listDir, 0, 1, true, true)) {
			Dispatcher::getInstance().addTask(createTask(boost::bind(&Game::playerAutoWalk,
				this, player->getID(), listDir)));
			SchedulerTask* task = createSchedulerTask(player->getStepDuration(), boost::bind(&Game::playerBrowseField, this,
				playerId, pos));
	...
	if (it == browseFields.end()) {
		...
		createSchedulerTask(30000, boost::bind(&Game::decreaseBrowseFieldRef, this, tile->getPosition()));
	} else {
		container = it->second;
	}
...
void Game::playerSeekInContainer(uint32_t playerId, uint8_t containerId, uint16_t index)
{
	...
	player->sendContainer(containerId, container, (container->getParent() != NULL));//, index);
}
	...
	uint32_t tradePlayerId, uint16_t spriteId)
{
	...
	{
		std::stringstream ss;
		ss << tradePartner->getName() << Localization::t(player->getLanguage(), " tells you to move closer.");
		player->sendTextMessage(MSG_INFO_DESCR, ss.str());
		return false;
	}

	Item* tradeItem = dynamic_cast<Item*>(internalGetThing(player, pos, stackpos, spriteId, STACKPOS_USE));
	if(!tradeItem || tradeItem->getClientID() != spriteId || !tradeItem->isPickupable() || !player->canAcceptTradeItem(tradeItem) ||
        (tradeItem->isLoadedFromMap() && (tradeItem->getUniqueId() != 0 || (tradeItem->getActionId() != 0 && tradeItem->getContainer()))))
	{
		player->sendCancelMessage(RET_NOTPOSSIBLE);
		return false;
	}
...
bool Game::internalStartTrade(Player* player, Player* tradePartner, Item* tradeItem)
{
	...
	{
		char buffer[100];
		sprintf(buffer, Localization::t(tradePartner->getLanguage(), "%s wants to trade with you").c_str(), player->getName().c_str());
	...
	else
	{
		...
		CreatureEventList tradeEvents = player->getCreatureEvents(CREATURE_EVENT_TRADE_REQUEST_ACCEPT);
	    for(CreatureEventList::iterator it = tradeEvents.begin(); it != tradeEvents.end(); ++it) {
		    (*it)->executeTradeRequestAccept(player, tradePartner, tradeItem, counterOfferItem);
	    }
	}
...
bool Game::playerAcceptTrade(uint32_t playerId)
{
	...
	if(ret1 == RET_NOERROR && ret2 == RET_NOERROR)
	{
		...
		{			
			...
			uint32_t count1 = tradeItem1->getItemCount(); // PS This item count need be getted before, cuz auto grouping can do problem
			uint32_t count2 = tradeItem2->getItemCount();

			ret1 = internalMoveItem(player, cylinder1, tradePartner, INDEX_WHEREEVER, tradeItem1, count1, NULL);
			if (ret1 == RET_NOERROR) {
	...
	if(!isSuccess)
	{
		std::string errorDescription = getTradeErrorDescription(ret1, tradeItem1, tradePartner);
		tradePartner->sendTextMessage(MSG_INFO_DESCR, errorDescription);
		tradeItem2->onTradeEvent(ON_TRADE_CANCEL, tradePartner, NULL);

		errorDescription = getTradeErrorDescription(ret2, tradeItem2, player);
...
std::string Game::getTradeErrorDescription(ReturnValue ret, Item* item, Player* player)
{
	std::stringstream ss;
	if(ret == RET_NOTENOUGHCAPACITY)
	{
		ss << Localization::t(player->getLanguage(), "You do not have enough capacity to carry");
		if(item->isStackable() && item->getItemCount() > 1)
			ss << Localization::t(player->getLanguage(), " these objects.");
		else
			ss << Localization::t(player->getLanguage(), " this object.");

		ss << std::endl << " " << item->getWeightDescription();
	}
	else if(ret == RET_NOTENOUGHROOM || ret == RET_CONTAINERNOTENOUGHROOM)
	{
		ss << Localization::t(player->getLanguage(), "You do not have enough room to carry");
		if(item->isStackable() && item->getItemCount() > 1)
			ss << Localization::t(player->getLanguage(), " these objects.");
		else
			ss << Localization::t(player->getLanguage(), " this object.");
		...
		ss << Localization::t(player->getLanguage(), "Trade could not be completed.");
...
bool Game::playerLookInTrade(uint32_t playerId, bool lookAtCounterOffer, int32_t index)
{
	...
	std::stringstream ss;
	ss << Localization::t(player->getLanguage(), "You see ");

	int32_t lookDistance = 1;  /* <- Show all details std::max(std::abs(player->getPosition().x - tradeItem->getPosition().x),
		std::abs(player->getPosition().y - tradeItem->getPosition().y));*/
	if(!index)
	{
		ss << tradeItem->getDescription(lookDistance, player);
	...
	while(listContainer.size() > 0)
	{
		...
		for(it = container->getItems(); it != container->getEnd(); ++it)
		{
			...
			ss << (*it)->getDescription(lookDistance, player);
			if(player->hasCustomFlag(PlayerCustomFlag_CanSeeItemDetails))
			{
...
bool Game::playerLookInShop(uint32_t playerId, uint16_t spriteId, uint8_t count)
{
	...
	std::stringstream ss;
	ss << Localization::t(player->getLanguage(), "You see ") << Item::getDescription(it, 1, NULL, subType, player);
	if(player->hasCustomFlag(PlayerCustomFlag_CanSeeItemDetails))
...
bool Game::playerLookAt(uint32_t playerId, const Position& pos, uint16_t spriteId, int16_t stackpos)
{
	...
	std::stringstream ss;
	ss << Localization::t(player->getLanguage(), "You see ") << thing->getDescription(lookDistance, player);
	if(player->hasCustomFlag(PlayerCustomFlag_CanSeeItemDetails))
	{
		if(Item* item = thing->getItem())
		{
	...
	if (const Creature* creature = thing->getCreature()) {
        ss << std::endl << Localization::t(player->getLanguage(), "Health: [") << creature->getHealth() << " / " << creature->getMaxHealth() << "]"; // PS - Now all players can see creature's health
        ...
        if(player->hasCustomFlag(PlayerCustomFlag_CanSeeCreatureDetails))
	    {
			...
			{
				ss << std::endl << "IP: " << convertIPAddress(destPlayer->getIP()) << ", Client: " << destPlayer->getClientName() << " (" << destPlayer->getClientVersion() << ").";
	...
	if(player->hasCustomFlag(PlayerCustomFlag_CanSeePosition))
		ss << std::endl << Localization::t(player->getLanguage(), "Position: [X: ") << thingPos.x << "] [Y: " << thingPos.y << "] [Z: " << thingPos.z << "].";
...
bool Game::playerQuestInfo(uint32_t playerId, uint16_t questId)
{
	...
	bool deny = false;
	CreatureEventList events = player->getCreatureEvents(CREATURE_EVENT_QUEST_INFO);
	for(CreatureEventList::iterator it = events.begin(); it != events.end(); ++it)
	{
		if(!(*it)->executeQuestInfo(player, quest->getId()) && !deny) {
		    deny = true;
        }
	}
...
bool Game::playerChangeOutfit(uint32_t playerId, Outfit_t outfit)
{
	...
	if (player->getRequestedCustomOutfit()) {
        player->hasRequestedCustomOutfit(false);
        
        CreatureEventList events = player->getCreatureEvents(CREATURE_EVENT_CUSTOM_OUTFIT);
		for(CreatureEventList::iterator it = events.begin(); it != events.end(); ++it) {
			(*it)->executeCustomOutfit(player, outfit);
		}
...
{    
	...
	if (player->hasCondition(CONDITION_SLEEP, 0)) {
        player->sendCancelMessage(RET_NOTPOSSIBLE);
        return false;
    }
	...
	{
		char buffer[75];
		sprintf(buffer, Localization::t(player->getLanguage(), "You are still muted for %d seconds.").c_str(), muteTime);
...
//    if(!player->getAccess() &&
//        (regex_search(text, boost::regex("^(?:http://)?([^/]+)(?:/?.*/?)/(.*)$")) ||
//        regex_search(text, boost::regex("www\\.[a-z]+[a-z0-9]*\\.(.*)")))) // PS - Hack link protection
//	{
//	    player->sendCancelMessage(RET_NOTPOSSIBLE);
//        return false;
//    }
...
{
	/*if (player->getLevel() < 10) // PS
	{
        player->sendTextMessage(MSG_STATUS_SMALL, "Only players with level 10 or higher can send private messages.");
		return false;
    } */
		...
		&& !player->hasFlag(PlayerFlag_CannotBeMuted))
	{
		...
		if(!canSee)
			...
			sprintf(buffer, Localization::t(player->getLanguage(), "Sorry, %s is currently ignoring private messages.").c_str(), toPlayer->getName().c_str());
	...
	char buffer[80];
	sprintf(buffer, Localization::t(player->getLanguage(), "Message sent to %s.").c_str(), toPlayer->getName().c_str());
	player->sendTextMessage(MSG_STATUS_SMALL, buffer);
	
	if (toPlayer->getDueling()) {
        player->sendCreatureSay(toPlayer, type, Localization::t(toPlayer->getLanguage(), "This player is dueling right now!"));
    }
    else if (toPlayer->getIsAfk()) {
        player->sendCreatureSay(toPlayer, type, (toPlayer->getAfkMessage() != "" ? ("AFK - " + toPlayer->getAfkMessage()) : Localization::t(toPlayer->getLanguage(), "This player is away in this moment.")));
    }
...
bool Game::playerTalkToChannel(Player* player, SpeakClasses type, const std::string& text, uint16_t channelId)
{
	switch(type)
	{
		case SPEAK_CHANNEL_Y:
		{
			if((channelId == CHANNEL_HELP || channelId == 8 || channelId == 7 || channelId == 10 || channelId == 11 || channelId == 12) && player->hasFlag(PlayerFlag_TalkOrangeHelpChannel)) // PS help and gcs
...
bool Game::publicPlayerTalkToChannel(Player* player, SpeakClasses type, const std::string& text, uint16_t channelId)
{
...
bool Game::isSightDirectClear(const Position& fromPos, const Position& toPos, bool floorCheck)
{
	return map->checkSightLineDirect(fromPos, toPos, floorCheck);
}

bool Game::internalCreatureTurn(Creature* creature, Direction dir)
{
	if(creature->hasCondition(CONDITION_SLEEP, 0))
	    return false;
...
bool Game::internalCreatureSay(Creature* creature, SpeakClasses type, const std::string& text,
	bool ghostMode, SpectatorVec* spectators/* = NULL*/, Position* pos/* = NULL*/, bool translate/* = false*/)
{
	...
	for(it = list.begin(); it != list.end(); ++it)
	{
		...
		if(!ghostMode || tmpPlayer->canSeeCreature(creature))
			tmpPlayer->sendCreatureSay(creature, type, (translate ? Localization::t(tmpPlayer->getLanguage(), text) : text), &destPos);
	}
...
bool Game::getPathToEx(const Creature* creature, const Position& targetPos,
	std::list<Direction>& dirList, const FindPathParams& fpp, bool extendedNodes /*= false*/)
{
	return map->getPathMatching(creature, dirList, FrozenPathingConditionCall(targetPos), fpp, extendedNodes);
}
...
bool Game::getPathToEx(const Creature* creature, const Position& targetPos, std::list<Direction>& dirList,
	...
	bool clearSight /*= true*/, int32_t maxSearchDist /*= -1*/, bool extendedNodes /*= false*/)
{
	FindPathParams fpp;
	...
	return getPathToEx(creature, targetPos, dirList, fpp, extendedNodes);
}
...
void Game::updateCreatureWalk(uint32_t creatureId)
{
	...
	if(creature && creature->getHealth() > 0)
	{
        if(creature->isFollowingPosition)
        {
	...
	int32_t& healthChange, bool checkDefense, bool checkArmor)
{
	...
	{
		// PS addMagicEffect(list, targetPos, MAGIC_EFFECT_POFF, target->isGhost());
		return true;
	}
	...
	{
		// PS addMagicEffect(list, targetPos, MAGIC_EFFECT_POFF);
		return true;
	}
	else if(blockType == BLOCK_ARMOR)
	{
		// PS addMagicEffect(list, targetPos, MAGIC_EFFECT_BLOCKHIT);
		return true;
	}
	...
	switch(combatType)
	{
        ...
        {
             effect = MAGIC_EFFECT_SMALLCLOUDS; // PS
             break;
        }
		...
		if(damage != 0)
		{
			...
			if(damage > 0)
			{
				...
				switch(combatType)
				{
					...
					case COMBAT_ENERGYDAMAGE:
					{
						textColor = TEXTCOLOR_GREY; //TEXTCOLOR_PURPLE;
						magicEffect = MAGIC_EFFECT_NONE; //MAGIC_EFFECT_ENERGY_DAMAGE;
						//tmp disabled splash = Item::CreateItem(13315);
						break;
					}
					...
					{
						textColor = TEXTCOLOR_LIGHTGREEN;
						magicEffect = MAGIC_EFFECT_NONE; //MAGIC_EFFECT_POISON_RINGS;
						//tmp disabled splash = Item::CreateItem(ITEM_SMALLSPLASH, FLUID_GREEN);
						break;
					}
					...
					case COMBAT_DROWNDAMAGE:
					{
						...
						magicEffect = MAGIC_EFFECT_NONE; //MAGIC_EFFECT_LOSE_ENERGY;
						//tmp disabled splash = Item::CreateItem(2016);
						break;
					}
					...
					{
						textColor = TEXTCOLOR_ORANGE;
						magicEffect = MAGIC_EFFECT_NONE; //MAGIC_EFFECT_HITBY_FIRE;
						//tmp disabled splash = Item::CreateItem(13314);
						break;
					}
					...
					case COMBAT_ICEDAMAGE:
					{
						...
						magicEffect = MAGIC_EFFECT_NONE; //MAGIC_EFFECT_ICEATTACK;
						//tmp disabled splash = Item::CreateItem(13316);
						break;
					}
					...
					case COMBAT_HOLYDAMAGE:
					{
						...
						magicEffect = MAGIC_EFFECT_NONE; //MAGIC_EFFECT_HOLYDAMAGE;
						//tmp disabled splash = Item::CreateItem(13317);
						break;
					}
					...
					case COMBAT_DEATHDAMAGE:
					{
						...
						magicEffect = MAGIC_EFFECT_NONE; //MAGIC_EFFECT_SMALLCLOUDS;
						break;
					}
					...
					{
						textColor = TEXTCOLOR_RED;
						magicEffect = MAGIC_EFFECT_NONE; //MAGIC_EFFECT_WRAPS_RED;
						break;
					}
                    ...
                    case COMBAT_POISON:
                    {
                        textColor = TEXTCOLOR_DARKPURPLE;
                        magicEffect = MAGIC_EFFECT_NONE;
                        //tmp disabled splash = Item::CreateItem(18102);
                        break;
                    }
                    ...
                    case COMBAT_GROUND:
                    {
                        ...
                        magicEffect = MAGIC_EFFECT_NONE;
                        //tmp disabled splash = Item::CreateItem(18101);
                        break;
                    }
                    ...
                    case COMBAT_GRASS:
                    {
                        ...
                        magicEffect = MAGIC_EFFECT_NONE;
                        //tmp disabled splash = Item::CreateItem(18100);
                        break;
                    }
                    
                    case COMBAT_ELECTRIC:
                    {
                        textColor = TEXTCOLOR_YELLOW;
                        magicEffect = MAGIC_EFFECT_NONE;
                        //tmp disabled splash = Item::CreateItem(13317);
				...
				{
					char buffer[20];
					sprintf(buffer, "-%d", damage); // PS

					addAnimatedText(list, targetPos, textColor, buffer);
				}
				
				if (splash && !findItemOfType(target->getTile(), splash->getID(), false, -1)) {
                    internalAddItem(NULL, target->getTile(), splash, INDEX_WHEREEVER, FLAG_NOLIMIT);
                    startDecay(splash);
				}
			}
...
void Game::addCreatureUpdateName(const Creature* creature)
{
	const SpectatorVec& list = getSpectators(creature->getPosition());
	...
	for(SpectatorVec::const_iterator it = list.begin(); it != list.end(); ++it)
	{
		if((player = (*it)->getPlayer()))
			player->sendCreatureUpdateName(creature);
	}
}
...
void Game::addCreatureJump(const Creature* creature)
{
    const SpectatorVec& list = getSpectators(creature->getPosition());
    Player* player = NULL;
	for(SpectatorVec::const_iterator it = list.begin(); it != list.end(); ++it)
	{
        ...
        if(player && player->isUsingOtclient())
			player->sendCreatureJump(creature);
	}
}

void Game::addCreatureEffect(const Creature* creature, uint8_t effectId, uint32_t var)
{
    const SpectatorVec& list = getSpectators(creature->getPosition());
    Player* player = NULL;
	for(SpectatorVec::const_iterator it = list.begin(); it != list.end(); ++it)
	{
		player = (*it)->getPlayer();
        if(player && player->isUsingOtclient())
			player->sendCreatureEffect(creature, effectId, var);
	}
}
...
void Game::internalDecayItem(Item* item)
{
	...
	{
        addMagicEffect(item->getTile()->getPosition(), MAGIC_EFFECT_POFF); // PS
        ReturnValue ret = internalRemoveItem(NULL, item);
		if(ret != RET_NOERROR)
...
int32_t Game::getRealLightHour() {
    if (lastLightCheck <= 0) {
        return getLightHour();
    }
    
    int32_t realLightHour = getLightHour() + ((time(NULL) - lastLightCheck) * (lightHourDelta / 10));
	if (realLightHour > 1440) {
		realLightHour = realLightHour - 1440;
    }
    
    return realLightHour;
}
...
bool Game::playerInviteToParty(uint32_t playerId, uint32_t invitedId)
{
	...
	{
		char buffer[90];
		sprintf(buffer, Localization::t(player->getLanguage(), "%s is already in a party.").c_str(), invitedPlayer->getName().c_str());
	...
	std::string comment, std::string statement, uint32_t statementId, bool ipBanishment)
{
	...
	{
		char buffer[90];
		sprintf(buffer, Localization::t(player->getLanguage(), "The comment may not exceed limit of %d characters.").c_str(), commentSize);
...
std::string Game::getSearchString(const Position fromPos, const Position toPos, Player* player, bool fromIsCreature/* = false*/, bool toIsCreature/* = false*/)
{
	/*
	...
	switch(distance)
	{
		case DISTANCE_BESIDE:
		{
			switch(level)
			{
				case LEVEL_SAME:
				{
					ss << Localization::t(player->getLanguage(), "is ");
					if(toIsCreature)
						ss << Localization::t(player->getLanguage(), "standing ");

					ss << Localization::t(player->getLanguage(), "next to you");
					break;
				}

				case LEVEL_HIGHER:
				{
					ss << Localization::t(player->getLanguage(), "is above ");
					if(fromIsCreature)
						ss << Localization::t(player->getLanguage(), "you");
				...
				case LEVEL_LOWER:
				{
					ss << Localization::t(player->getLanguage(), "is below ");
					if(fromIsCreature)
						ss << Localization::t(player->getLanguage(), "you");
		...
		case DISTANCE_CLOSE:
		{
			...
			{
				case LEVEL_SAME:
					ss << Localization::t(player->getLanguage(), "is to the");
					break;
				case LEVEL_HIGHER:
					ss << Localization::t(player->getLanguage(), "is on a higher level to the");
					break;
				case LEVEL_LOWER:
					ss << Localization::t(player->getLanguage(), "is on a lower level to the");
					break;
				default:
					break;
			}
		...
		case DISTANCE_FAR:
			ss << Localization::t(player->getLanguage(), "is far to the");
			break;

		case DISTANCE_VERYFAR:
			ss << Localization::t(player->getLanguage(), "is very far to the");
			break;
	...
	if(distance != DISTANCE_BESIDE)
	{
		...
		{
			case DIR_N:
				ss << Localization::t(player->getLanguage(), "north");
				break;

			case DIR_S:
				ss << Localization::t(player->getLanguage(), "south");
				break;

			case DIR_E:
				ss << Localization::t(player->getLanguage(), "east");
				break;

			case DIR_W:
				ss << Localization::t(player->getLanguage(), "west");
				break;

			case DIR_NE:
				ss << Localization::t(player->getLanguage(), "north-east");
				break;

			case DIR_NW:
				ss << Localization::t(player->getLanguage(), "north-west");
				break;

			case DIR_SE:
				ss << Localization::t(player->getLanguage(), "south-east");
				break;

			case DIR_SW:
				ss << Localization::t(player->getLanguage(), "south-west");
				break;
...
Highscore Game::getHighscore(uint16_t skill)
{
	...
	{
		if(skill == SKILL__MAGLEVEL)
			query << "SELECT `maglevel`, `name` FROM `players` WHERE `players`.`world_id` =" << g_config.getNumber(ConfigManager::WORLD_ID) << " ORDER BY `maglevel` DESC, `manaspent` DESC LIMIT " << g_config.getNumber(ConfigManager::HIGHSCORES_TOP);
		else
			query << "SELECT `level`, `name` FROM `players` WHERE `players`.`world_id` =" << g_config.getNumber(ConfigManager::WORLD_ID) << " ORDER BY `level` DESC, `experience` DESC LIMIT " << g_config.getNumber(ConfigManager::HIGHSCORES_TOP);
	...
	{
		query << "SELECT `player_skills`.`value`, `players`.`name` FROM `player_skills`,`players` WHERE `player_skills`.`skillid`=" << skill << " AND `player_skills`.`player_id`=`players`.`id` AND `players`.`world_id` =" << g_config.getNumber(ConfigManager::WORLD_ID) << " ORDER BY `player_skills`.`value` DESC, `player_skills`.`count` DESC LIMIT " << g_config.getNumber(ConfigManager::HIGHSCORES_TOP);
		if(!(result = db->storeQuery(query.str())))
			return hs;
...
bool Game::reloadInfo(ReloadInfo_t reload, uint32_t playerId/* = 0*/)
{
	...
	switch(reload)
	{
		...
		case RELOAD_ALL:
		{
            ...
            */
            std::cout << "[Warning - Game::reloadInfo] Reload all type does not work." << std::endl;
            done = true;
			break;
		}
...
void Game::showHotkeyUseMessage(Player* player, Item* item)
{
	...
	if(count == 1)
		sprintf(buffer, Localization::t(player->getLanguage(), "Using the last %s...").c_str(), it.name.c_str());
	else
		sprintf(buffer, Localization::t(player->getLanguage(), "Using one of %d %s...").c_str(), count, it.pluralName.c_str());

	player->sendTextMessage(MSG_INFO_DESCR, buffer);
}

bool Game::partyDuelJoin(uint32_t leaderId, uint32_t invitedId) // PS
{
    Player* invited = getPlayerByID(invitedId);
	if(!invited || invited->isRemoved())
		return false;

    Player* leader = getPlayerByID(leaderId);
	if(!leader || leader->isRemoved() || !leader->isPartyDuelInviting(invited))
		return false;

    if(!invited->getPartyDuel())
	{
		if(PartyDuel* partyDuel = leader->getPartyDuel())
            return partyDuel->join(invited, partyDuel->getPlayerTeam(leader));
    }

    invited->sendTextMessage(MSG_INFO_DESCR, "You are already in a party duel.");
	return false;
}

bool Game::partyDuelInvite(uint32_t leaderId, uint32_t invitedId) // PS
{
    Player* leader = getPlayerByID(leaderId);
	...
	Player* invited = getPlayerByID(invitedId);
	if(!invited || invited->isRemoved() || invited->isPartyDuelInviting(leader))
		return false;

	if(invited->getPartyDuel())
	{
		char buffer[90];
		sprintf(buffer, Localization::t(leader->getLanguage(), "%s is already in a party duel.").c_str(), invited->getName().c_str());
		leader->sendTextMessage(MSG_INFO_DESCR, buffer);
		return false;
	}

	PartyDuel* partyDuel = leader->getPartyDuel();
	if (!partyDuel) {
		//partyDuel = new PartyDuel(leader, 1, 1);
		return false;
    }
	else if(partyDuel->getLeaderA() != leader && partyDuel->getLeaderB() != leader)
		return false;
		
	if (partyDuel->getBet() > 0) {
		if (invited->getLevel() < 50) {
            leader->sendTextMessage(MSG_INFO_DESCR, "Only players level 50 or higher can be invited to duels with betting.");
		    return false;
        }
        else if (invited->hasCondition(CONDITION_INFIGHT)) {
            leader->sendTextMessage(MSG_INFO_DESCR, "You can't invite players with infight state.");
            return false;
        }
	}

    return partyDuel->invitePlayer(invited, partyDuel->getPlayerTeam(leader));
}

bool Game::partyDuelLeave(uint32_t playerId) // PS
{
    Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved())
		return false;

	if(!player->getPartyDuel() || player->hasCondition(CONDITION_INFIGHT) || !player->getPartyDuel()->getCancelable())
		return false;

	return player->getPartyDuel()->leave(player);
}

bool Game::partyDuelCreate(uint32_t leaderId, uint8_t maxMembers, uint8_t maxPokemon, bool cancelable, TournamentBattle* tournamentBattle, uint32_t bet) // PS
{
    Player* leader = getPlayerByID(leaderId);
	if(!leader || leader->isRemoved())
		return false;
		
	if (bet > 0 && leader->getLevel() < 50) {
		leader->sendTextMessage(MSG_INFO_DESCR, "Only players level 50 or higher can create duels with betting.");
		return false;
	}
    
    if(PartyDuel* partyDuel = new PartyDuel(leader, maxMembers, maxPokemon, cancelable, tournamentBattle, bet)) {
        partyDuels.push_back(partyDuel);
        return true;
    }
    return false;    
}

PvpArena* Game::getPvpArenaById(uint8_t id)
{
    PvpArenaMap::iterator it = pvpArenaMap.find(id);
	if(it != pvpArenaMap.end())
	    return it->second;
    return NULL;
}

bool Game::pvpArenaJoin(uint32_t playerId, uint8_t arenaId, uint8_t teamId)
{
    Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved())
		return false;

    if(!player->getPvpArena())
	{
		if(PvpArena* pvpArena = getPvpArenaById(arenaId))
            return pvpArena->join(player, (pvpArenaTeam_t)teamId);
    }

	return false;
}

bool Game::pvpArenaLeave(uint32_t playerId)
{
    Player* player = getPlayerByID(playerId);
	if(!player || player->isRemoved())
		return false;

    if(PvpArena* pvpArena = player->getPvpArena())
        return pvpArena->leave(player);
        
	return false;
}

bool Game::pvpArenaCreate(uint8_t id, const Position& blueSpawn, const Position& redSpawn, const Position& exit, uint32_t maxLevel, uint16_t blueChannel, uint16_t redChannel)
{    
    if(PvpArena* pvpArena = new PvpArena(id, blueSpawn, redSpawn, exit, maxLevel, blueChannel, redChannel))
    {
        pvpArenaMap[id] = pvpArena;
        return true;
    }
    return false;
}

void Game::emergencySave()
{
    std::cout << "> Emergency Save... ";
    if (emergencySaved) {
    ...
    std::cout << "Players online now: " << Player::autoList.size() << std::endl;
    uint16_t count = 0;
	...
	{
		count++;
        std::cout << ">> Saving [" << count << "] " << it->second->getGUID();
        it->second->loginPosition = it->second->getPosition();
    ...
    if (map->saveMap()) {
        ...
        std::cout << "Error! The Map could not be saved!" << std::endl;
    }

	std::cout << ">> EMERGENCY SAVE: Complete." << std::endl;
}

Creature* Game::getCreatureByNickname(std::string s)
{
	if(s.empty())
		return NULL;
	...
	for(AutoList<Creature>::iterator it = autoList.begin(); it != autoList.end(); ++it)
	{
		if(!it->second->isRemoved() && asLowerCaseString(it->second->getNickname()) == s)
			return it->second;
	}
...
void Game::checkPartyDuels()
{ 
    for(PartyDuelVector::iterator it = partyDuels.begin(); it != partyDuels.end();) {
        if ((*it)->getEnded()) {
            delete (*it);
            it = partyDuels.erase(it);
        }
        else {
            (*it)->onThink();
            ++it;
    ...
    Scheduler::getInstance().addEvent(createSchedulerTask(
		EVENT_PARTYDUELINTERVAL, boost::bind(&Game::checkPartyDuels, this)));   
}

ReturnValue Game::swapItems(Item* itemA, Item* itemB)
{
	if (itemA == itemB) {
        return RET_NOERROR;
    }
    
    Cylinder* cylinderA = itemA->getParent();
    if (!cylinderA) {
        return RET_NOTPOSSIBLE;
    }
    
    int32_t itemAIndex = cylinderA->__getIndexOfThing(itemA);
    if (itemAIndex == -1) {
        return RET_NOTPOSSIBLE;
    }
    
    if (cylinderA->__queryRemove(itemA, 1, 0) != RET_NOERROR) {
        return RET_NOTPOSSIBLE;
    }
    
    Cylinder* cylinderB = itemB->getParent();
    if (!cylinderB) {
        return RET_NOTPOSSIBLE;
    }
    
    int32_t itemBIndex = cylinderB->__getIndexOfThing(itemB);
    if (itemBIndex == -1) {
        return RET_NOTPOSSIBLE;
    }
    
    if (cylinderB->__queryRemove(itemB, 1, 0) != RET_NOERROR) {
        return RET_NOTPOSSIBLE;
    }

    cylinderA->postRemoveNotification(NULL, itemA, cylinderA, itemAIndex, true);
    cylinderA->__replaceThing(itemAIndex, itemB);
	cylinderA->postAddNotification(NULL, itemB, cylinderA, itemAIndex);
	itemA->setParent(cylinderB);
	
	cylinderB->postRemoveNotification(NULL, itemB, cylinderB, itemBIndex, true);
    cylinderB->__replaceThing(itemBIndex, itemA);
	cylinderB->postAddNotification(NULL, itemA, cylinderB, itemBIndex);
	itemB->setParent(cylinderA);
	return RET_NOERROR;
}

void Game::logOnlinePlayers()
{ 
    IODatalog::getInstance()->logOnlinePlayers(g_config.getNumber(ConfigManager::WORLD_ID), time(NULL), getPlayersOnline());
    Scheduler::getInstance().addEvent(createSchedulerTask(
		EVENT_LOGONLINEPLAYERSINTERVAL, boost::bind(&Game::logOnlinePlayers, this)));   
}
/*
void Game::logPing()
{ 
    uint64_t date = time(NULL);
    for(AutoList<Player>::iterator it = Player::autoList.begin(); it != Player::autoList.end(); ++it) {
		if(!it->second->isRemoved()) {
            IODatalog::getInstance()->logPing(it->second->getGUID(), date,
                (it->second->getLastPong() - it->second->getLastPing()) / 10);
		}
	}

    Scheduler::getInstance().addEvent(createSchedulerTask(
		EVENT_LOGPINGINTERVAL, boost::bind(&Game::logPing, this)));   
}
*/
void Game::playerAddStatistic(Player* player, PlayerStatistic_t statistic, uint32_t value)
{
    uint32_t newValue = IOPlayerStatistics::getInstance()->saveStatistic(player->getGUID(), statistic, value);
    
    if (newValue > 0) {
	    CreatureEventList events = player->getCreatureEvents(CREATURE_EVENT_STATISTIC_CHANGE);
	    for(CreatureEventList::iterator it = events.begin(); it != events.end(); ++it) {
		    (*it)->executeStatisticChange(player, statistic, newValue);
	    }
    }
}
...
void Game::playerCreateMarketOffer(uint32_t playerId, uint8_t type, uint16_t spriteId, uint16_t amount, uint32_t price, bool anonymous)
{
	...
	uint64_t fee = (uint64_t)((price / 100.) * amount);
	if (fee < 20) {
		fee = 20;
	} else if (fee > 1000) {
		fee = 1000;
	...
	if (type == MARKETACTION_SELL) {
		if (fee > player->balance) {
            player->sendCancel("You do not have enough money at bank to pay the fee.");
			return;
		}

		Depot* depotChest = player->getDepot(player->getLastDepotId(), false);
		if (!depotChest) {
			return;
		}
		...
		containers.push_back(depotChest);
		bool enough = false;
		...
		do {
			...
			for (int32_t i = 0; i < (int32_t)container->size(); ++i) {
			    Item* item = container->getItem(i);
			    if (!item) {
                    std::cout << "WARNING - Game::playerCreateMarketOffer Unknown item at index " << i << std::endl;
                    continue;
                }
				...
				count += Item::countByType(item, -1, false);
				if (count >= amount) {
					enough = true;
					break;
				}
		...
		if (it.stackable) {
			uint16_t tmpAmount = amount;
 			for (ItemList::iterator it = itemList.begin(); it != itemList.end(); ++it) {
				uint16_t removeCount = std::min<uint16_t>(tmpAmount, (*it)->getItemCount());
				tmpAmount -= removeCount;
				internalRemoveItem(NULL, (*it), removeCount);
...
void Game::playerCancelMarketOffer(uint32_t playerId, uint32_t timestamp, uint16_t counter)
{
	...
	if (offer.type == MARKETACTION_BUY) {
		player->balance += static_cast<uint64_t>(offer.price) * offer.amount;
		player->sendMarketEnter(player->getLastDepotId());
	} else {
		...
		if (it.stackable) {
			...
			while (tmpAmount > 0) {
				...
				if (internalAddItem(player, player->getDepot(player->getLastDepotId(), false), item, INDEX_WHEREEVER, FLAG_NOLIMIT) != RET_NOERROR) {
					delete item;
					break;
				}
	...
	playerBrowseMarketOwnOffers(playerId); // update offers - yes, thats ugly, but will work for now
}
	...
	if (offer.type == MARKETACTION_BUY) {
		...
		do {
	        ...
	        for (int32_t i = 0; i < (int32_t)container->size(); ++i) {
			    Item* item = container->getItem(i);
			    if (!item) {
                    std::cout << "WARNING - Game::playerAcceptMarketOffer Unknown item at index " << i << std::endl;
                    continue;
                }
		...
		if (it.stackable) {
			...
			while (tmpAmount > 0) {
				...
				Item* item = Item::CreateItem(it.id, stackCount);
				if (internalAddItem(buyerPlayer, buyerPlayer->getDepot(player->getLastDepotId(), false), item, INDEX_WHEREEVER, FLAG_NOLIMIT) != RET_NOERROR) {
					delete item;
					break;
				}
		...
		Player* sellerPlayer = getPlayerByGuidEx(offer.playerId);
		if (sellerPlayer) {
			sellerPlayer->balance += totalPrice;
			
			if (sellerPlayer->isVirtual()) {
			    IOLoginData::getInstance()->savePlayer(sellerPlayer);
			    delete sellerPlayer;
            }
		}
...
void Game::checkExpiredMarketOffers()
{
    ...
    for (ExpiredMarketOfferList::const_iterator it = expiredBuyOffers.begin(); it != expiredBuyOffers.end(); ++it) {
		uint64_t totalPrice = static_cast<uint64_t>((*it).price) * (*it).amount;

		Player* player = getPlayerByGuidEx((*it).playerId);
		if (player) {
			player->balance += totalPrice;
		...
		IOMarket::moveOfferToHistory((*it).id, OFFERSTATE_EXPIRED);
	}

	const ExpiredMarketOfferList& expiredSellOffers = IOMarket::getExpiredOffers(MARKETACTION_SELL);
	for (ExpiredMarketOfferList::const_iterator it = expiredSellOffers.begin(); it != expiredSellOffers.end(); ++it) {
		Player* player = getPlayerByGuidEx((*it).playerId);
		if (!player) {
			continue;
		}
		...
		if (itemType.stackable) {
			...
			for (uint16_t i = 0; i < (*it).amount; ++i) {
...
bool Game::playerRequestPollWindow(uint32_t playerId)
{
	Player* player = getPlayerByID(playerId);
	if (!player || player->isRemoved() || !Polls::getInstance()->getAvailablePollForAccount(player->getAccount())) {
		return false;
    }

	player->sendPollWindow();
	return true;
}

bool Game::playerPollVote(uint32_t playerId, uint8_t pollVote)
{
    Player* player = getPlayerByID(playerId);
	if (!player || player->isRemoved() || !Polls::getInstance()->getAvailablePollForAccount(player->getAccount())) {
		return false;
    ...
    if (Polls::getInstance()->doComputePollVote(player->getAccount(), pollVote)) {
        player->sendCancel("Your vote has been computed.");
        return true;
    }
    else {
        player->sendCancel("Your vote can't be computed.");
    }

	return false;
}

bool Game::playerPollVoteText(uint32_t playerId, std::string text)
{
    Player* player = getPlayerByID(playerId);
	if (!player || player->isRemoved() || !Polls::getInstance()->getAvailablePollForAccount(player->getAccount())) {
		return false;
    ...
    if (Polls::getInstance()->doComputePollVote(player->getAccount(), text)) {
        player->sendCancel("Your vote has been computed.");
        return true;
    }
    else {
...
{    
    ...
    for (uint16_t z = 0; z < (uint16_t)MAP_MAX_LAYERS; z++) {
		for(uint16_t y = 1; y <= map->mapHeight; y++) {
			for(uint16_t x = 1; x <= map->mapWidth; x++) {
				if (!(tile = getTile(x, y, z)) || !tile->getItemList()) {
					continue;
                }
				
                TileItemVector* tileItems = tile->getItemList();
                if (tileItems) {
			        for (ItemVector::iterator it = tileItems->getBeginDownItem(); it != tileItems->getEndDownItem(); ++it) {
				        if ((*it)->isMoveable() && !(*it)->isLoadedFromMap() && !(*it)->isScriptProtected()) {
						    items.push_back(*it);
                        }
					}
                }
	...
	IODatalog::getInstance()->logMapItems(items);
}
