...
Condition* Condition::createCondition(ConditionId_t _id, ConditionType_t _type, int32_t _ticks, int32_t param/* = 0*/, bool _buff/* = false*/, uint32_t _subId/* = 0*/)
{
    switch((int32_t)_type)
	{
		...
		case CONDITION_SLEEP:
            return new ConditionSleep(_id, _type, _ticks, _buff, _subId);
            
        case CONDITION_CUSTOM:
            return new ConditionCustom(_id, _type, _ticks, _buff, _subId, param & 0xFF);
            
        case CONDITION_EXTRASPATK:
            return new ConditionExtraSpAtk(_id, _type, _ticks, _buff, _subId, param & 0xFF);
            
        case CONDITION_EXTRASPDEF:
             return new ConditionExtraSpDef(_id, _type, _ticks, _buff, _subId, param & 0xFF);
             
        case CONDITION_INVULNERABLE:
            return new ConditionInvulnerable(_id, _type, _ticks, _buff, _subId);
            
        case CONDITION_DRUNK:
            return new ConditionDrunk(_id, _type, _ticks, _buff, _subId);
	...
	//{
		if(internalHealthTicks >= healthTicks)
		{
			...
			if (showEffect) {
			    ...
			    g_game.addAnimatedText(creature->getPosition(), TEXTCOLOR_LIGHTGREEN, buffer);
			    g_game.addMagicEffect(creature->getPosition(), MAGIC_EFFECT_WRAPS_GREEN);
            }
		}
...
void ConditionRegeneration::endCondition(Creature* creature, ConditionEnd_t reason)
{
    ConditionGeneric::endCondition(creature, reason);
    if (reason == CONDITIONEND_TICKS && subId == 0) {
        g_game.addMagicEffect(creature->getPosition(), 168);
    }
}
...
bool ConditionSpeed::executeCondition(Creature* creature, int32_t interval)
{
    ...
    if(internalTicks >= ticks)
    {
        internalTicks = 0;       
        if (speedDelta < 0) {
            g_game.addMagicEffect(creature->getPosition(), 11); /* Yellow Energize */
        } else if (!creature->getPlayer()) { // We dont show this to players because its used on Ride/Fly/Dive/Surf/Bike, the effect is ugly in those situations, so lets disable for players
            g_game.addMagicEffect(creature->getPosition(), 480); /* Orange Energize */
        }       
    }
...
ConditionSleep::ConditionSleep(ConditionId_t _id, ConditionType_t _type, int32_t _ticks, bool _buff, uint32_t _subId): // PS
ConditionGeneric(_id, _type, _ticks, _buff, _subId)
{
...
void ConditionSleep::addCondition(Creature* creature, const Condition* addCondition)
{
	if(!updateCondition(addCondition))
		return;

	setTicks(addCondition->getTicks());
	const ConditionSleep& conditionSleep = static_cast<const ConditionSleep&>(*addCondition);

	sleepTicks = conditionSleep.sleepTicks;
}

bool ConditionSleep::unserializeProp(ConditionAttr_t attr, PropStream& propStream)
{
	switch(attr)
	{
		case CONDITIONATTR_TICKS:
...
bool ConditionSleep::serialize(PropWriteStream& propWriteStream)
{
	if(!ConditionGeneric::serialize(propWriteStream))
		return false;

	propWriteStream.ADD_UCHAR(CONDITIONATTR_TICKS);
	propWriteStream.ADD_VALUE(sleepTicks);
...
bool ConditionSleep::executeCondition(Creature* creature, int32_t interval)
{
    ...
    if(internalSleepTicks >= sleepTicks)
    {
        internalSleepTicks = 0;       
        g_game.addMagicEffect(creature->getPosition(), 26);        
    }

	return ConditionGeneric::executeCondition(creature, interval);
}

bool ConditionSleep::setParam(ConditionParam_t param, int32_t value)
{
	bool ret = ConditionGeneric::setParam(param, value);
	switch(param)
	{
		case CONDITIONPARAM_EFFECTTICKS:
			sleepTicks = value;
			return true;
...
ConditionCustom::ConditionCustom(ConditionId_t _id, ConditionType_t _type, int32_t _ticks, bool _buff, uint32_t _subId, uint16_t _effect):
Condition(_id, _type, _ticks, _buff, _subId)
{
	effect = _effect;
	internalTicks = 0;
...
bool ConditionCustom::executeCondition(Creature* creature, int32_t interval)
{
	internalTicks += interval;
	if(internalTicks >= ticks)
    {
        ...
        if (effect != MAGIC_EFFECT_NONE) {       
            g_game.addMagicEffect(creature->getPosition(), effect);        
        }
    }
...
void ConditionCustom::addCondition(Creature* creature, const Condition* addCondition)
{
	if(updateCondition(addCondition))
	{
		setTicks(addCondition->getTicks());
		const ConditionCustom& conditionCustom = static_cast<const ConditionCustom&>(*addCondition);

		effect = conditionCustom.effect;
		internalTicks = 0;
		ticks = conditionCustom.ticks;
	}
}

bool ConditionCustom::setParam(ConditionParam_t param, int32_t value)
{
	bool ret = Condition::setParam(param, value);
	switch(param)
	{
...
bool ConditionCustom::unserializeProp(ConditionAttr_t attr, PropStream& propStream)
{
	switch(attr)
	{
		case CONDITIONATTR_LIGHTLEVEL:
...
bool ConditionCustom::serialize(PropWriteStream& propWriteStream)
{
	if(!Condition::serialize(propWriteStream))
		return false;
		
	propWriteStream.ADD_UCHAR(CONDITIONATTR_LIGHTLEVEL);
	propWriteStream.ADD_VALUE(effect);
...
ConditionExtraSpAtk::ConditionExtraSpAtk(ConditionId_t _id, ConditionType_t _type, int32_t _ticks, bool _buff, uint32_t _subId, int32_t extraSpAtkAmount):
Condition(_id, _type, _ticks, _buff, _subId)
{
...
bool ConditionExtraSpAtk::startCondition(Creature* creature)
{
	internalTicks = 0;

	if (Monster* monster = creature->getMonster())
	   monster->addExtraSpAtk(extraSpAtk);
	
	return Condition::startCondition(creature);
}

bool ConditionExtraSpAtk::executeCondition(Creature* creature, int32_t interval)
{
	internalTicks += interval;
	if(internalTicks >= ticks)
    {
        internalTicks = 0;       
        g_game.addMagicEffect(creature->getPosition(), 199);        
    }

	return Condition::executeCondition(creature, interval);
}

void ConditionExtraSpAtk::endCondition(Creature* creature, ConditionEnd_t reason)
{
	if (Monster* monster = creature->getMonster())
	    monster->addExtraSpAtk(-extraSpAtk);
}

void ConditionExtraSpAtk::addCondition(Creature* creature, const Condition* addCondition)
{
	if(updateCondition(addCondition))
	{
		setTicks(addCondition->getTicks());
		const ConditionExtraSpAtk& conditionExtraSpAtk = static_cast<const ConditionExtraSpAtk&>(*addCondition);

		internalTicks = 0;
		ticks = conditionExtraSpAtk.ticks;
	}
}

bool ConditionExtraSpAtk::setParam(ConditionParam_t param, int32_t value)
{
	bool ret = Condition::setParam(param, value);
	switch(param)
	{
...
bool ConditionExtraSpAtk::unserializeProp(ConditionAttr_t attr, PropStream& propStream)
{
	switch(attr)
	{
		case CONDITIONATTR_LIGHTLEVEL:
...
bool ConditionExtraSpAtk::serialize(PropWriteStream& propWriteStream)
{
	if(!Condition::serialize(propWriteStream))
		return false;
		
	propWriteStream.ADD_UCHAR(CONDITIONATTR_LIGHTLEVEL);
	propWriteStream.ADD_VALUE(extraSpAtk);
...
ConditionExtraSpDef::ConditionExtraSpDef(ConditionId_t _id, ConditionType_t _type, int32_t _ticks, bool _buff, uint32_t _subId, int32_t extraSpDefAmount):
Condition(_id, _type, _ticks, _buff, _subId)
{
...
bool ConditionExtraSpDef::startCondition(Creature* creature)
{
	internalTicks = 0;

	if (Monster* monster = creature->getMonster())
	   monster->addExtraSpDef(extraSpDef);
	
	return Condition::startCondition(creature);
}

bool ConditionExtraSpDef::executeCondition(Creature* creature, int32_t interval)
{
	internalTicks += interval;
	if(internalTicks >= ticks)
    {
        internalTicks = 0;       
        g_game.addMagicEffect(creature->getPosition(), 485); /* Blue Energize */        
    }

	return Condition::executeCondition(creature, interval);
}

void ConditionExtraSpDef::endCondition(Creature* creature, ConditionEnd_t reason)
{
	if (Monster* monster = creature->getMonster())
	    monster->addExtraSpDef(-extraSpDef);
}

void ConditionExtraSpDef::addCondition(Creature* creature, const Condition* addCondition)
{
	if(updateCondition(addCondition))
	{
		setTicks(addCondition->getTicks());
		const ConditionExtraSpDef& conditionExtraSpDef = static_cast<const ConditionExtraSpDef&>(*addCondition);

		internalTicks = 0;
		ticks = conditionExtraSpDef.ticks;
	}
}

bool ConditionExtraSpDef::setParam(ConditionParam_t param, int32_t value)
{
	bool ret = Condition::setParam(param, value);
	switch(param)
	{
...
bool ConditionExtraSpDef::unserializeProp(ConditionAttr_t attr, PropStream& propStream)
{
	switch(attr)
	{
		case CONDITIONATTR_LIGHTLEVEL:
...
bool ConditionExtraSpDef::serialize(PropWriteStream& propWriteStream)
{
	if(!Condition::serialize(propWriteStream))
		return false;
		
	propWriteStream.ADD_UCHAR(CONDITIONATTR_LIGHTLEVEL);
	propWriteStream.ADD_VALUE(extraSpDef);
...
ConditionInvulnerable::ConditionInvulnerable(ConditionId_t _id, ConditionType_t _type, int32_t _ticks, bool _buff, uint32_t _subId): // PS
ConditionGeneric(_id, _type, _ticks, _buff, _subId)
{
	internalTicks = 0;
	ticks = 1000;
...
void ConditionInvulnerable::addCondition(Creature* creature, const Condition* addCondition)
{
	if(!updateCondition(addCondition))
		return;

	setTicks(addCondition->getTicks());
	const ConditionInvulnerable& conditionInvulnerable = static_cast<const ConditionInvulnerable&>(*addCondition);

	ticks = conditionInvulnerable.ticks;
}

bool ConditionInvulnerable::unserializeProp(ConditionAttr_t attr, PropStream& propStream)
{
	switch(attr)
	{
		case CONDITIONATTR_TICKS:
...
bool ConditionInvulnerable::serialize(PropWriteStream& propWriteStream)
{
	if(!ConditionGeneric::serialize(propWriteStream))
		return false;
...
bool ConditionInvulnerable::executeCondition(Creature* creature, int32_t interval)
{
	internalTicks += interval;
    ...
    {
        internalTicks = 0;       
        g_game.addMagicEffect(creature->getPosition(), 38);        
    }

	return ConditionGeneric::executeCondition(creature, interval);
}

bool ConditionInvulnerable::setParam(ConditionParam_t param, int32_t value)
{
	bool ret = ConditionGeneric::setParam(param, value);
	switch(param)
	{
...
ConditionDrunk::ConditionDrunk(ConditionId_t _id, ConditionType_t _type, int32_t _ticks, bool _buff, uint32_t _subId): // PS
ConditionGeneric(_id, _type, _ticks, _buff, _subId)
{
	internalTicks = 0;
	ticks = 1000;
...
void ConditionDrunk::addCondition(Creature* creature, const Condition* addCondition)
{
	if(!updateCondition(addCondition))
		return;
	...
	ticks = conditionDrunk.ticks;
}

bool ConditionDrunk::unserializeProp(ConditionAttr_t attr, PropStream& propStream)
{
	switch(attr)
	{
		case CONDITIONATTR_TICKS:
...
bool ConditionDrunk::serialize(PropWriteStream& propWriteStream)
{
	if(!ConditionGeneric::serialize(propWriteStream))
		return false;
...
bool ConditionDrunk::executeCondition(Creature* creature, int32_t interval)
{
    ...
    {
        internalTicks = 0;       
        g_game.addMagicEffect(creature->getPosition(), 46); /* Stars */        
    }
