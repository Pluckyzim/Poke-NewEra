...
{
	if (/*item->isMoveable() || */item->getContainer()) {
		BrowseFieldMap::iterator it = g_game.browseFields.find(this);
...
{
    if (/*newItem->isMoveable() || */newItem->getContainer()) {
		BrowseFieldMap::iterator it = g_game.browseFields.find(this);
		if (it != g_game.browseFields.end()) {
	...
	} else if (/*oldItem->isMoveable() || */oldItem->getContainer()) {
		BrowseFieldMap::iterator it = g_game.browseFields.find(this);
		if (it != g_game.browseFields.end()) {
...
{    
	...
	{        
		...
		{            
            ...
            {
                if(!monster->isSummon()) // wild monster can't enter in protection zones
                    return RET_NOTPOSSIBLE;
                    
                if(const Player* monsterMaster = monster->getMaster()->getPlayer()) // this isn't wild but let's check if the master is battleing
                {
                    if(monsterMaster->isPzLocked())
                        return RET_NOTPOSSIBLE;
                }
            }
	...
	else if(const Item* item = thing->getItem())
	{
		...
		if(!getHouseTile())
		{
		    ...
		    {
                if(tmpItem->getUniqueOwner())
                    return RET_NOTPOSSIBLE;
                else if(Container* itemContainer = tmpItem->getContainer())
                {
