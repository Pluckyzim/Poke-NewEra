...
bool Spawns::parseSpawnNode(xmlNodePtr p, bool checkDuplicate)
{
	...
	while(tmpNode)
	{
		if(!xmlStrcmp(tmpNode->name, (const xmlChar*)"monster"))
		{
			...
			spawn->addMonster(name, placePos, direction, interval, level);
		}
...
bool Spawn::spawnMonster(uint32_t spawnId, MonsterType* mType, const Position& pos, Direction dir, uint32_t level, bool startup /*= false*/)
{
	Monster* monster = NULL;
//    if (mType->shiny != "" && random_range(0, g_config.getNumber(ConfigManager::SHINY_APPEAR_CHANCE)) == 0)
    if (random_range(0, g_config.getNumber(ConfigManager::SHINY_APPEAR_CHANCE)) == 0) {
        monster = Monster::createMonster("Shiny " + mType->name);
    }
	...
	bool result = true;
	CreatureEventList events = monster->getCreatureEvents(MONSTER_EVENT_SINGLE_SPAWN);
	for (CreatureEventList::iterator it = events.begin(); it != events.end(); ++it) {
	    if (!(*it)->executeSingleSpawn(monster) && result) {
	        result = false;
        }
    }
    ...
    g_creatureEvents->monsterSpawn(monster);
    ...
    /*if (Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_INVULNERABLE, 3000)) {
...
void Spawn::startup()
{
	for(SpawnMap::iterator it = spawnMap.begin(); it != spawnMap.end(); ++it)
	{
		...
		spawnMonster(it->first, sb.mType, sb.pos, sb.direction, sb.level, true);
	}
}
...
void Spawn::checkSpawn()
{
	...
	for(SpawnedMap::iterator it = spawnedMap.begin(); it != spawnedMap.end();)
	{
		...
		/*
		This is buggy, with this spawns can be dubled with the same Monster
		...
		if(spawnedMap.count(spawnId) == 0)
		{
			if(OTSYS_TIME() >= sb.lastSpawn + sb.interval)
			{
				...
				*/
                ...
                spawnMonster(spawnId, sb.mType, sb.pos, sb.direction, sb.level);
				++spawnCount;
...
bool Spawn::addMonster(const std::string& _name, const Position& _pos, Direction _dir, uint32_t _interval, uint32_t level)
{    
