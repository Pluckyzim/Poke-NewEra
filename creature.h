...
class Creature : public AutoId, virtual public Thing
{
	...
	public:
		...
		virtual double getDamageRatioByMaster(Creature* master) const;
		...
		virtual Position getFollowPosition() const {return followPosition;}
		virtual bool setFollowPosition(Position position);
		...
		Npc* getNpcMaster() const {return isNpcSummon() ? master->getNpc() : NULL;}
		Creature* getTopMaster();
		const Creature* getTopMaster() const;
		...
		virtual double getGainedExperience(Creature* attacker, Item* referenceItem) const {return getDamageRatio(attacker, referenceItem) * (double)getLostExperience();}
		void addDamagePoints(Creature* attacker, int32_t damagePoints);
		void addHealPoints(Creature* caster, int32_t healthPoints);
		bool hasBeenAttacked(uint32_t attackerId, bool checkTicks) const;
		...
		virtual void onAttackedCreatureKilled(Creature* target, Item* referenceItem = NULL);
		...
		virtual CreatureIcons_t getCreatureIcon() const {return icon;}
		virtual void setCreatureIcon(CreatureIcons_t newIcon) {icon = newIcon;}
        ...
        void onReferencedItemRemoved(Item* item);
        void giveKillersExperience();
	...
	protected:
		...
		struct CountBlock_t
		{
			...
			CountBlock_t(uint32_t points)
			{
				...
				total = points;
				referenceItem = NULL;
				referenceItemOwner = 0;
			}

			CountBlock_t() {start = ticks = total = 0; referenceItem = NULL; referenceItemOwner = 0;}
		};
		...
		virtual double getDamageRatio(Creature* attacker, Item* referenceItem = NULL) const;
