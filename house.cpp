...
void House::updateDoorDescription(std::string _name/* = ""*/)
{
	...
	if(owner)
	{
		...
		sprintf(houseDescription, "It belongs to %s '%s'. Nobody owns this %s. It costs %d dollars.", tmp.c_str(), name.c_str(), tmp.c_str(), price);
...
bool House::transferToDepot()
{
	...
	ItemList moveList;
	CreatureVector houserNpcs;
	
	for(HouseTileList::iterator it = houseTiles.begin(); it != houseTiles.end(); ++it)
	{
		for(uint32_t i = 0; i < (*it)->getThingCount(); ++i)
		{
			if (Creature* creature = (*it)->__getThing(i)->getCreature()) {
                if (creature->getNpc() && creature->getNpc()->getHouser()) {
                    houserNpcs.push_back(creature);
                    continue;
                }
            }
            ...
            if(!(item = (*it)->__getThing(i)->getItem()))
 			...
 			if(item->isPickupable() || item->forceTransferToDepot())
	...
	for (it = houserNpcs.begin(); it != houserNpcs.end(); ++it) {
	...
	if(player)
	{
        ...
        if (receiver) {
            for(ItemList::iterator it = moveList.begin(); it != moveList.end(); ++it) {
			    if ((*it)->getUniqueOwner() > 0 && !player->isItemUniqueOwner((*it)->getUniqueOwner())) {                                                                       
                    Player* playerOwner = g_game.getPlayerByGuidEx((*it)->getUniqueOwner());
	                if (playerOwner) {
                        Depot* depotOwner = playerOwner->getDepot(0, true);
                        if (depotOwner) {
                            g_game.internalMoveItem(NULL, (*it)->getParent(), depotOwner, INDEX_WHEREEVER, (*it),
                                (*it)->getItemCount(), NULL, ((*it)->forceTransferToDepot() ? FLAG_NOLIMIT | FLAG_IGNORENOTMOVEABLE : FLAG_NOLIMIT));
                        }
                        
                        if (playerOwner->isVirtual()) {
		                    IOLoginData::getInstance()->savePlayer(playerOwner);
		                    delete playerOwner;
                        }
	                }
                ...
                ReturnValue r = RET_NOERROR;
                if ((*it)->forceTransferToDepot()) {
                    r = g_game.internalMoveItem(NULL, (*it)->getParent(), receiver, INDEX_WHEREEVER, (*it), (*it)->getItemCount(), NULL, FLAG_NOLIMIT | FLAG_IGNORENOTMOVEABLE);
                }
                else {
                    r = g_game.internalMoveItem(NULL, (*it)->getParent(), receiver, INDEX_WHEREEVER, (*it), (*it)->getItemCount(), NULL, FLAG_NOLIMIT);
                }
                ...
                if (r != RET_NOERROR && depot) { // If we cant send to the parcel, lets try to send directly into the depot. This problem ocurr when the player doenst open the depot and receive unique items. Unique items cant be added to a container that the player isnt carrying or isnt at the depot.
                    if ((*it)->forceTransferToDepot()) {
                        g_game.internalMoveItem(NULL, (*it)->getParent(), depot, INDEX_WHEREEVER, (*it), (*it)->getItemCount(), NULL, FLAG_NOLIMIT | FLAG_IGNORENOTMOVEABLE);
                    }
                    else {
...
bool Houses::payRent(Player* player, House* house, uint32_t bid, time_t _time/* = 0*/)
{
	...
	else if(Depot* depot = player->getDepot(0/*town->getID()*/, true)) // PS default town id, unique depot
...
bool Houses::payHouse(House* house, time_t _time, uint32_t bid)
{
    ...
    if(rentPeriod == RENTPERIOD_NEVER || house->getPaidUntil() > _time ||
        !house->getRent()) {
        if (player->isVirtual()) {
            delete player;
        }
	...
	if(!paid && _time >= (house->getLastWarning() + 86400))
	{
		...
		{
			Depot* depot = player->getDepot(0/*town->getID()*/, true); // PS default town id, unique depot
			...
			if(depot && letter)
			{
				...
				std::stringstream s;
				s << "Warning!\nThe " << period << " rent of " << house->getRent() << " dollar for your "
