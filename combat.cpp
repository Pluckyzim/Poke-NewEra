...
ReturnValue Combat::canDoCombat(const Creature* attacker, const Creature* target)
{
	...
	if(target->hasCondition(CONDITION_INVULNERABLE)) // PS
	...
	if(const Player* targetPlayer = target->getPlayer())
	{
		...
		{
            if((attackerPlayer->getPvpArena() && targetPlayer->getPvpArena() && attackerPlayer->getPvpArenaTeam() != targetPlayer->getPvpArenaTeam()) ||
               (attackerPlayer->getSurviveArena() && targetPlayer->getSurviveArena()))
			    return RET_NOERROR;
            return RET_YOUMAYNOTATTACKTHISCREATURE;
            /*
            checkZones = true;
			...
			if(target->getMaster())
			{
				...
				else if(const Player* targetMaster = target->getTopMaster()->getPlayer())
				{
                    if((attackerPlayer->getDueling() && attackerPlayer->isOpponentPartyDuel(targetMaster)) ||
                       ...
                       (attackerPlayer->getSurviveArena() && targetMaster->getSurviveArena()))
                        return RET_NOERROR;
                }
                else if(const Npc* npc = target->getTopMaster()->getNpc()) // PS
                {
                    if(npc->getOpponent() && npc->getOpponent() == attackerPlayer)
                        return RET_NOERROR;
                }
                else if(const Monster* targetMaster = target->getTopMaster()->getMonster()) {
                    return RET_NOERROR;
                }
                return RET_YOUMAYNOTATTACKTHISCREATURE;
			}
		}
		else if(!target->isSummon() && !attacker->isSummon())
		    return RET_YOUMAYNOTATTACKTHISCREATURE;
        else if(target->isSummon() && target->getPlayerMaster())
        {
            const Npc* attackerNpc = NULL;
            if((attackerNpc = attacker->getNpc()) || (attacker->getMaster()
                && (attackerNpc = attacker->getMaster()->getNpc())))
            {
                if(attackerNpc->getOpponent() && attackerNpc->getOpponent() != target->getPlayerMaster())
                    return RET_YOUMAYNOTATTACKTHISCREATURE;
            }
        }
	}
...
ReturnValue Combat::canTargetCreature(const Player* player, const Creature* target)
{
	...
	{
		// PS if(player->getSecureMode() == SECUREMODE_ON)
		...
		for(size_t x = 0; x < cols; ++x)
		{
			if(area->getValue(y, x) != 0)
			{
				if(targetPos.z < MAP_MAX_LAYERS && g_game.isSightClear(targetPos, Position(tmpX, tmpY, targetPos.z), true) &&
                    g_game.isSightClear(centerPos, Position(tmpX, tmpY, targetPos.z), true))
				{
	...
	if(ownerId && !getTile()->hasFlag(TILESTATE_PVPZONE))
	{
		if(Creature* owner = g_game.getCreatureByID(ownerId))
		{
			...
			if(!harmful || (OTSYS_TIME() - createTime) <= (uint32_t)g_config.getNumber(
				ConfigManager::FIELD_OWNERSHIP) || creature->hasBeenAttacked(ownerId, true))
